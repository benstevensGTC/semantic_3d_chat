"""Evaluate V92's sealed scene-one retention/conversation repair candidate.
...
V92 wordings for each chat intent, and thirteen zero-payload causal controls.
...
from semantic_3d_chat.evaluation.v92_scene1_retention_conversation_preflight import (
    held_wording_rows_v92 as _preflight_held_wording_rows_v92,
)
from semantic_3d_chat.evaluation.v92_scene1_retention_conversation_preflight import (
    primary_rows_v92 as _preflight_primary_rows_v92,
)
...
PREDICTIONS_ARTIFACT: Final[str] = "gemma4_v92_scene1_retention_conversation_repair_predictions_v1"
EVALUATION_ARTIFACT: Final[str] = "gemma4_v92_scene1_retention_conversation_repair_evaluation_v1"
...
_QUESTION_ID: Final[re.Pattern[str]] = re.compile(
    r"v(?:91|92)_(.+)_(?:existing_0[0-5]|new_held_0[01])"
)


def _intent_id_v92(record_or_row: Mapping[str, Any] | RowV73) -> str:
    ...
    if value in INTENT_IDS:
        return str(value)
    raise ValueError(f"Malformed V92 conversational question ID: {question_id}")


def primary_rows_v92(config: Mapping[str, Any]) -> tuple[RowV73, ...]:
    """Return the exact thirteen development-known primary questions."""

    rows = _preflight_primary_rows_v92(config)
    if (
        len(rows) != 13
        or len({row.question_id for row in rows}) != 13
        or {_intent_id_v92(row) for row in rows} != set(INTENT_IDS)
        or any(not row.question_id.startswith("v91_") for row in rows)
    ):
        raise ValueError("V92 primary conversational inventory changed")
    return rows


def held_wording_rows_v92(config: Mapping[str, Any]) -> tuple[RowV73, ...]:
    """Return exactly twenty-six entirely new V92 wording-only holds."""

    rows = _preflight_held_wording_rows_v92(config)
    if (
        len(rows) != 26
        or len({row.question_id for row in rows}) != 26
        or Counter(_intent_id_v92(row) for row in rows)
        != Counter({identifier: 2 for identifier in INTENT_IDS})
        or any(not row.question_id.startswith("v92_") for row in rows)
    ):
        raise ValueError("V92 new held-wording inventory changed")
    return rows
...
def _with_intents(
    ...
    return [{**record, "intent_id": _intent_id_v92(record)} for record in records]


def score_records_v92(
    ...
    *,
    ...
    """Score V92 gates while retaining the audited V91 answer semantics."""
    ...
    *,
    ...
    for ordinal, row in enumerate(rows, 1):
        ...
        if phase != "canonical":
            ...
            record["intent_id"] = _intent_id_v92(row)
        records.append(record)
        if ordinal == 1 or ordinal % 12 == 0 or ordinal == len(rows):
            print(
                json.dumps(
                    {
                        "event": "v92_evaluation_row",
                        "phase": phase,
                        "ordinal": ordinal,
                        "total": len(rows),
                        "question_id": row.question_id,
...
def run_evaluation_v92(config_path: str | Path = CONFIG) -> dict[str, Any]:
    """Run create-once V92 fixed-final evaluation with local Gemma/MPS."""
    ...
    training_bindings = authenticate_training_report_v92(config, config_path=config_path)
    canonical_rows = load_canonical_rows_v92(config)
    primary_rows = primary_rows_v92(config)
    held_rows = held_wording_rows_v92(config)
    ...
    if predictions_path.exists() or report_path.exists():
        raise FileExistsError("V92 create-once evaluation output exists")

    cpu_memory, memory_hash_before, _metadata = load_scene1_memory_v86(config)
    if tuple(cpu_memory.shape) != EXPECTED_PREFIX_SHAPE:
        raise ValueError("V92 immutable scene-memory shape changed")
    ...
    with audit:
        ...
        if language.device.type != "mps":
            raise RuntimeError("V92 fixed-final evaluation requires local MPS")
        collection = install_lora_banks(language.model, combined_lora_settings_v92(runtime, config))
        if not isinstance(collection, LoRABankCollection):
            raise TypeError("V92 evaluation LoRA installation failed")
        frozen_source = load_frozen_parent_v92(
            collection,
            config["sources"]["parent_v89_checkpoint"],
            config,
        )
        candidate = load_fixed_final_bridge_v92(
        ...
        if (
            ...
            raise ValueError("V92 loaded candidate differs from authenticated training evidence")
        collection.eval()
        ...
        if len(frozen_states_before) != 13:
            raise ValueError("V92 requires the exact frozen thirteen-bank parent")
        ...
        canonical_records = _evaluate_rows_v92(
            language=language,
        ...
        primary_records = _evaluate_rows_v92(
            language=language,
            system_prompt=system_prompt,
            memory=memory,
            memory_hash=memory_hash_before,
        ...
        held_records = _evaluate_rows_v92(
            language=language,
            system_prompt=system_prompt,
            memory=memory,
            memory_hash=memory_hash_before,
    ...
    if len(all_records) != EXPECTED_EVALUATED_QUESTION_COUNT:
        raise RuntimeError("V92 evaluated question inventory changed")
    if len(causal_records) != EXPECTED_CAUSAL_CONTROL_COUNT:
        raise RuntimeError("V92 causal-control inventory changed")
    prefix_hash_invariant = bool(
    ...
    predictions = {
        ...
        "new_v92_held_wording_row_count": len(held_records),
        ...
        "new_v92_held_wording_records": held_records,
    ...
    report = {
        ...
        "frozen_thirteen_bank_parent_mutated": not parent_state_invariant,
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    report = run_evaluation_v92(args.config)
    print(
        json.dumps(
            {
