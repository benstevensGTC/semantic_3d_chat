"""Validate bounded learning-rate reports and attest their saved checkpoints.

Ordinary selection is report-only and never loads a model checkpoint or oracle
artifact.  The separate V14 attestation path deliberately reads the four pinned
epoch-4 checkpoints, recomputes their trainable-bank hashes, and verifies their
optimizer state and final gates against the historical sweep summary.  Neither
path reads oracle artifacts.
"""
...
class ExpectedSweepContract:
    ...
    source_head_commit: str = "1ee8b5d13777e74ebdfe1f87e7d8320403ad5fbf"
    source_head_tree: str = "b606e85cbb5a786ba2e00f971cf07c174bc5cbef"
...
# These are the four predeclared V14 screen arms.  The short config hashes are
# generated by the training configuration loader and are meaningful only in
# conjunction with the fully pinned source tree above and the raw checkpoint
# hashes recorded by the attestation manifest.
EXPECTED_V14_ARMS: dict[str, dict[str, Any]] = {
    "lr1e4": {
        "learning_rate": 1e-4,
        "config_hash": "7c4523140639",
        "output_namespace": "gemma4_color_mirror_decoder_banks_v14_lr1e4",
    },
    "lr3e4": {
        "learning_rate": 3e-4,
        "config_hash": "63744feeea44",
        "output_namespace": "gemma4_color_mirror_decoder_banks_v14_lr3e4",
    },
    "lr1e3": {
        "learning_rate": 1e-3,
        "config_hash": "5dd4eb4176e2",
        "output_namespace": "gemma4_color_mirror_decoder_banks_v14_lr1e3",
    },
    "lr2e3": {
        "learning_rate": 2e-3,
        "config_hash": "93ff12019b76",
        "output_namespace": "gemma4_color_mirror_decoder_banks_v14_lr2e3",
    },
}
V14_TRAINABLE_BANK_NAME = "extension_v13"
...
def _file_sha256(path: Path) -> str:
    """Hash one evidence file without buffering it all in memory."""
...
def _decode_json_strict(raw: bytes) -> Any:
...
def _load_json_strict(path: Path) -> tuple[Any, str]:
    raw = path.read_bytes()
    return _decode_json_strict(raw), hashlib.sha256(raw).hexdigest()
...
def summarize_lr_sweep(named_reports: Mapping[str, str | Path]) -> dict[str, Any]:
    ...
    for name in sorted(named_reports):
        ...
        input_report_sha256: str | None = None
        try:
            raw_report = raw_path.read_bytes()
            input_report_sha256 = hashlib.sha256(raw_report).hexdigest()
            report = _decode_json_strict(raw_report)
            arm = _validate_report(
                name,
                path,
                report,
                input_report_sha256=input_report_sha256,
        ...
        ranked_rows.append(
            {
                ...
                "name": arm["name"],
                "path": arm["path"],
                "input_report_sha256": arm["input_report_sha256"],
    ...
    return {
        ...
        "selected_report_sha256": (
            None if winner is None else winner["input_report_sha256"]
        ),
    }
...
def _require_exact_names(
    ...
    if observed != expected:
        _violation(
            ...
            f"{field} names differ: expected={sorted(expected)!r}, observed={sorted(observed)!r}",
        )


def _validate_pinned_v14_source(value: Any, field: str) -> dict[str, Any]:
    source = dict(_validate_source_provenance(value, field))
    expected = EXPECTED_SWEEP_CONTRACT
    if source["head_commit"] != expected.source_head_commit:
        _violation("invalid_provenance", f"{field}.head_commit is not the V14 source pin")
    if source["head_tree"] != expected.source_head_tree:
        _violation("invalid_provenance", f"{field}.head_tree is not the V14 source pin")
    return source


def _configured_lora_banks(value: Any, field: str) -> list[dict[str, Any]]:
    """Return architecture-only bank contracts from metadata or selection JSON."""

    lora = _mapping(value, field)
    if lora.get("schema_version") != 2 or lora.get("enabled") is not True:
        _violation("invalid_provenance", f"{field} must be enabled named-bank LoRA schema 2")
    banks = lora.get("banks")
    if not isinstance(banks, list) or not banks:
        _violation("invalid_provenance", f"{field}.banks must be a non-empty list")
    keys = (
        "name",
        "trainable",
        "rank",
    ...
    return [{key: deepcopy(_mapping(bank, field).get(key)) for key in keys} for bank in banks]


def _recompute_trainable_bank_hashes(
    adapter_path: Path,
    metadata: Mapping[str, Any],
) -> tuple[dict[str, str], dict[str, int], dict[str, int]]:
    """Hash trainable bank tensors directly from the safetensors checkpoint."""
    ...
    try:
        ...
        _violation("invalid_adapter_checkpoint", f"cannot load safetensors adapter: {exc}")
    banks = _configured_lora_banks(metadata.get("lora"), "metadata.lora")
    trainable = [bank for bank in banks if bank["trainable"] is True]
    if [bank["name"] for bank in trainable] != [V14_TRAINABLE_BANK_NAME]:
        _violation(
            "invalid_provenance",
            f"V14 must contain exactly the {V14_TRAINABLE_BANK_NAME!r} trainable bank",
        )
    expected_hashes = _mapping(
        metadata.get("lora_bank_state_sha256"), "metadata.lora_bank_state_sha256"
    )
    hashes: dict[str, str] = {}
    ...
    for bank in trainable:
        ...
        if not state:
            _violation("invalid_adapter_checkpoint", f"adapter has no tensors for bank {name!r}")
        if any(not bool(torch.isfinite(tensor).all()) for tensor in state.values()):
            _violation("invalid_adapter_checkpoint", f"adapter bank {name!r} is non-finite")
        hashes[name] = tensor_state_sha256(state)
        parameter_counts[name] = sum(int(tensor.numel()) for tensor in state.values())
        tensor_counts[name] = len(state)
        expected_hash = _sha256(
            expected_hashes.get(name), f"metadata.lora_bank_state_sha256.{name}"
        )
        if hashes[name] != expected_hash:
            _violation(
                "adapter_state_hash_mismatch",
                f"recomputed adapter bank {name!r} hash differs from metadata",
            )
        configured_count = next(
            (
                raw.get("adapter_parameter_count")
                for raw in metadata["lora"]["banks"]
                if raw.get("name") == name
            ),
            None,
        )
        if configured_count != parameter_counts[name]:
            _violation(
                "invalid_adapter_checkpoint",
                f"adapter bank {name!r} parameter count differs from metadata",
            )
    return hashes, parameter_counts, tensor_counts
...
def _validate_optimizer_checkpoint(
    ...
    *,
    ...
    """Validate the resumable optimizer payload without executing pickle code."""
    ...
    try:
        ...
        _violation("invalid_optimizer_checkpoint", f"cannot load optimizer state: {exc}")
    state = _mapping(payload, "optimizer")
    raw_groups = state.get("param_groups")
    raw_slots = state.get("state")
    if not isinstance(raw_groups, list) or len(raw_groups) != 1:
        _violation("invalid_optimizer_checkpoint", "optimizer must have one parameter group")
    slots = _mapping(raw_slots, "optimizer.state")
    group = _mapping(raw_groups[0], "optimizer.param_groups[0]")
    parameters = group.get("params")
    if not isinstance(parameters, list) or len(parameters) != parameter_tensor_count:
        _violation(
            "invalid_optimizer_checkpoint",
            "optimizer parameter tensor count differs from the trainable LoRA bank",
        )
    if set(parameters) != set(slots):
        _violation("invalid_optimizer_checkpoint", "optimizer slots do not cover its parameters")
    observed_lr = _finite_number(group.get("lr"), "optimizer.param_groups[0].lr")
    observed_decay = _finite_number(
        group.get("weight_decay"), "optimizer.param_groups[0].weight_decay"
    )
    if observed_lr != learning_rate or observed_decay != 0.0:
        _violation("invalid_optimizer_checkpoint", "optimizer hyperparameters differ from V14")
    observed_steps: set[int] = set()
    for parameter, raw_slot in slots.items():
        slot = _mapping(raw_slot, f"optimizer.state[{parameter!r}]")
        step = slot.get("step")
        try:
            numeric_step = float(step.item()) if hasattr(step, "item") else float(step)
        except (TypeError, ValueError, RuntimeError) as exc:
            _violation("invalid_optimizer_checkpoint", f"invalid optimizer step tensor: {exc}")
        if not math.isfinite(numeric_step) or not numeric_step.is_integer():
            _violation("invalid_optimizer_checkpoint", "optimizer step must be a finite integer")
        observed_steps.add(int(numeric_step))
    if observed_steps != {optimizer_step}:
        _violation("invalid_optimizer_checkpoint", "optimizer slots have incorrect update counts")
    return {
        "parameter_group_count": 1,
        "parameter_tensor_count": len(parameters),
        "slot_count": len(slots),
        "slot_optimizer_steps": sorted(observed_steps),
        "learning_rate": observed_lr,
        "weight_decay": observed_decay,
    }


def _attestation_report(
    metadata: Mapping[str, Any], selection: Mapping[str, Any]
) -> dict[str, Any]:
    """Combine independently stored evidence only for strict report validation."""

    report = deepcopy(dict(metadata))
    report.update(
        {
            "target_epochs": EXPECTED_SWEEP_CONTRACT.epochs,
            "epochs": metadata.get("epoch"),
            "steps": metadata.get("global_step"),
            "optimizer_steps": metadata.get("optimizer_step"),
            "selection": deepcopy(dict(selection)),
            "lora_optimizer": deepcopy(selection.get("lora_optimizer")),
        }
    )
    return report


def _compare_sweep_arm(
    name: str,
    ...
    for key in keys:
        if historical.get(key) != validated.get(key):
            _violation(
                "sweep_summary_mismatch",
                f"historical sweep arm {name!r} field {key!r} differs from checkpoint evidence",
            )


def attest_v14_lr_sweep(
    named_checkpoints: Mapping[str, str | Path],
    named_selections: Mapping[str, str | Path],
    sweep_summary_path: str | Path,
) -> dict[str, Any]:
    """Attest the V14 screen from epoch-4 checkpoints and selection reports.

    Historical training-report paths are deliberately not opened.  They are
    mutable convenience outputs, whereas the checkpoint directory contains the
    actual weights, optimizer state, post-update gate, and update counters.
    """

    expected_names = set(EXPECTED_V14_ARMS)
    _require_exact_names(named_checkpoints, expected_names, "checkpoint")
    _require_exact_names(named_selections, expected_names, "selection")
    summary_path = Path(sweep_summary_path)
    try:
        summary_raw, summary_sha256 = _load_json_strict(summary_path)
    except (OSError, UnicodeError, json.JSONDecodeError) as exc:
        _violation("unreadable_report", f"cannot read historical sweep summary: {exc}")
    summary = _mapping(summary_raw, "sweep_summary")
    if summary.get("schema_version") != 1:
        _violation("sweep_summary_mismatch", "historical sweep schema must be 1")
    raw_summary_arms = summary.get("arms")
    if not isinstance(raw_summary_arms, list):
        _violation("sweep_summary_mismatch", "historical sweep arms must be a list")
    summary_arms: dict[str, Mapping[str, Any]] = {}
    for index, raw_arm in enumerate(raw_summary_arms):
        arm = _mapping(raw_arm, f"sweep_summary.arms[{index}]")
        name = arm.get("name")
        if not isinstance(name, str) or name in summary_arms:
            _violation("sweep_summary_mismatch", "historical sweep arm names must be unique")
        summary_arms[name] = arm
    _require_exact_names(summary_arms, expected_names, "historical sweep arm")

    attested_arms: list[dict[str, Any]] = []
    validated_arms: list[dict[str, Any]] = []
    for name in sorted(expected_names):
        checkpoint = Path(named_checkpoints[name])
        selection_path = Path(named_selections[name])
        metadata_path = checkpoint / "metadata.json"
        adapter_path = checkpoint / "adapter.safetensors"
        optimizer_path = checkpoint / "optimizer.pt"
        try:
            metadata_raw, metadata_sha256 = _load_json_strict(metadata_path)
            selection_raw, selection_sha256 = _load_json_strict(selection_path)
            adapter_sha256 = _file_sha256(adapter_path)
            optimizer_sha256 = _file_sha256(optimizer_path)
        except (OSError, UnicodeError, json.JSONDecodeError) as exc:
            _violation("unreadable_checkpoint", f"cannot read {name!r} evidence: {exc}")
        metadata = _mapping(metadata_raw, f"{name}.metadata")
        selection = _mapping(selection_raw, f"{name}.selection")
        _assert_finite_tree(metadata, f"{name}.metadata")
        _assert_finite_tree(selection, f"{name}.selection")
        expected_arm = EXPECTED_V14_ARMS[name]
        expected = EXPECTED_SWEEP_CONTRACT
        if metadata.get("schema_version") != 3:
            _violation("protocol_violation", f"{name} checkpoint schema must be 3")
        counts = {
            "epoch": _positive_int(metadata.get("epoch"), f"{name}.epoch"),
            "global_step": _positive_int(metadata.get("global_step"), f"{name}.global_step"),
            "optimizer_step": _positive_int(
                metadata.get("optimizer_step"), f"{name}.optimizer_step"
            ),
            "gradient_accumulation": _positive_int(
                metadata.get("gradient_accumulation"), f"{name}.gradient_accumulation"
            ),
        }
        if counts != {
            "epoch": expected.epochs,
            "global_step": expected.steps,
            "optimizer_step": expected.optimizer_steps,
            "gradient_accumulation": expected.gradient_accumulation,
        }:
            _violation("protocol_violation", f"{name} checkpoint update counts differ from V14")
        if metadata.get("config_hash") != expected_arm["config_hash"]:
            _violation("invalid_provenance", f"{name} config hash differs from its V14 pin")
        if metadata.get("output_namespace") != expected_arm["output_namespace"]:
            _violation("invalid_provenance", f"{name} output namespace differs from V14")
        metadata_source = _validate_pinned_v14_source(
            metadata.get("source_provenance"), f"{name}.metadata.source_provenance"
        )
        selection_source = _validate_pinned_v14_source(
            selection.get("source_provenance"), f"{name}.selection.source_provenance"
        )
        if metadata_source != selection_source:
            _violation("invalid_provenance", f"{name} metadata and selection sources differ")
        if _configured_lora_banks(metadata.get("lora"), f"{name}.metadata.lora") != (
            _configured_lora_banks(selection.get("lora"), f"{name}.selection.lora")
        ):
            _violation("invalid_provenance", f"{name} metadata and selection LoRA configs differ")
        if selection.get("gradient_accumulation") != expected.gradient_accumulation:
            _violation("protocol_violation", f"{name} selection grad accumulation differs")
        if selection.get("freeze_scene_adapter") is not True:
            _violation("frozen_state_violation", f"{name} selection does not freeze scene adapter")

        evidence_report = _attestation_report(metadata, selection)
        validated = _validate_report(
            name,
            str(checkpoint),
            evidence_report,
            input_report_sha256=metadata_sha256,
        )
        if validated["learning_rate"] != expected_arm["learning_rate"]:
            _violation("protocol_violation", f"{name} learning rate differs from its V14 pin")
        recomputed_hashes, parameter_counts, tensor_counts = _recompute_trainable_bank_hashes(
            adapter_path, metadata
        )
        optimizer_validation = _validate_optimizer_checkpoint(
            optimizer_path,
            learning_rate=float(validated["learning_rate"]),
            optimizer_step=counts["optimizer_step"],
            parameter_tensor_count=tensor_counts[V14_TRAINABLE_BANK_NAME],
        )
        _compare_sweep_arm(name, validated, summary_arms[name])
        validated_arms.append(validated)
        attested_arms.append(
            {
                "name": name,
                "checkpoint_path": str(checkpoint),
                "historical_report_path": summary_arms[name].get("path"),
                "selection_path": str(selection_path),
                "config_hash": metadata["config_hash"],
                "output_namespace": metadata["output_namespace"],
                ...
                "source_provenance": metadata_source,
                "selection_sha256": selection["train"]["selected_ids_sha256"],
                "pair_membership_sha256": selection[
                    "training_counterfactual_pair_membership_sha256"
                ],
                "frozen_state": validated["frozen_state"],
                "recomputed_trainable_bank_state_sha256": recomputed_hashes,
                "recomputed_trainable_bank_parameter_count": parameter_counts,
                "optimizer_validation": optimizer_validation,
                "final_pair_gate_sha256": _canonical_sha256(metadata["pair_candidate_gate"]),
                "sweep_summary_correspondence": True,
            }
        )

    def rank_key(arm: Mapping[str, Any]) -> tuple[Any, ...]:
        metrics = _mapping(arm["mirror_metrics"], "mirror_metrics")
        return (
            *(-float(metrics[field]) for field in RANKING_FIELDS[:-1]),
            float(arm["learning_rate"]),
    ...
    ranked = sorted((arm for arm in validated_arms if arm["eligible"]), key=rank_key)
    ranked_names = [arm["name"] for arm in ranked]
    historical_ranking = summary.get("ranking")
    if not isinstance(historical_ranking, list) or [
        _mapping(row, "sweep_summary.ranking").get("name") for row in historical_ranking
    ] != ranked_names:
        _violation("sweep_summary_mismatch", "historical ranking differs from checkpoint gates")
    historical_contract = summary.get("canonical_provenance_contract_sha256")
    contracts = {arm["provenance_contract_sha256"] for arm in validated_arms}
    if len(contracts) != 1 or contracts != {historical_contract}:
        _violation("sweep_summary_mismatch", "historical provenance contract differs")
    if summary.get("report_count") != 4 or summary.get("eligible_count") != 4:
        _violation("sweep_summary_mismatch", "historical sweep arm counts differ")
    if summary.get("rejected_count") != 0:
        _violation("sweep_summary_mismatch", "historical sweep unexpectedly rejects an arm")
    if summary.get("ranked_winner") != ranked_names[0] or summary.get("selected_arm") != "lr2e3":
        _violation("sweep_summary_mismatch", "historical winner differs from checkpoint gates")
    if ranked_names[0] != "lr2e3":
        _violation("sweep_summary_mismatch", "checkpoint gates do not reproduce the V14 winner")
    return {
        "schema_version": 1,
        "attestation_type": "v14_epoch_004_checkpoint_sweep",
        "canonical_evidence": "checkpoint_plus_selection",
        "historical_training_reports_loaded": False,
        "expected_sweep_contract": asdict(EXPECTED_SWEEP_CONTRACT),
        "historical_sweep_summary": {
            "path": str(summary_path),
            "sha256": summary_sha256,
            "schema_version": summary["schema_version"],
            "canonical_provenance_contract_sha256": historical_contract,
            "ranked_winner": summary["ranked_winner"],
            "selected_arm": summary["selected_arm"],
        },
        "validated_arm_count": len(attested_arms),
        "all_arms_attested": True,
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    parser.add_argument(
        ...
        type=parse_named_report,
        metavar="NAME=DIR",
        help="named epoch-4 checkpoint for V14 attestation; repeat once per arm",
    )
    parser.add_argument(
        "--selection",
        action="append",
        type=parse_named_report,
        metavar="NAME=PATH",
        help="named selection report for V14 attestation; repeat once per arm",
    )
    parser.add_argument(
        "--sweep-summary",
        type=Path,
        help="historical V14 sweep summary whose final gates are being attested",
    )
    parser.add_argument("--output", required=True, type=Path, help="machine-readable JSON output")
    args = parser.parse_args(argv)
    attestation_requested = bool(args.checkpoint or args.selection or args.sweep_summary)
    if attestation_requested:
        if args.report:
            parser.error("--report cannot be combined with checkpoint attestation inputs")
        if not args.checkpoint or not args.selection or args.sweep_summary is None:
            parser.error("attestation requires --checkpoint, --selection, and --sweep-summary")
        checkpoints: dict[str, Path] = {}
        selections: dict[str, Path] = {}
        for label, bindings, destination in (
            ("checkpoint", args.checkpoint, checkpoints),
            ("selection", args.selection, selections),
        ):
            for name, path in bindings:
                if name in destination:
                    parser.error(f"duplicate {label} name: {name}")
                destination[name] = path
        summary = attest_v14_lr_sweep(checkpoints, selections, args.sweep_summary)
        write_summary(summary, args.output)
        print(
            json.dumps(
                {
                    "all_arms_attested": summary["all_arms_attested"],
                    "selected_arm": summary["selected_arm"],
                    "validated_arm_count": summary["validated_arm_count"],
                },
                sort_keys=True,
            )
        )
        return 0
    if not args.report:
        parser.error("at least one --report is required for ordinary sweep ranking")
