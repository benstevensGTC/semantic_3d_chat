"""Bounded V51 train-only query-alpha grid.

V51 follows the failed V50 scene-alpha grid.  It reconstructs the same guarded
normalized sign direction through exactly one V49 delegate reconstruction,
keeps the scene alpha fixed at 1.0, and evaluates query alphas 1.75, 2.25,
1.5, and 2.5 in declared order.  The V50 query-alpha-2.0 result is authenticated
as a nonselectable anchor and is not rerun as a candidate.  Every candidate is
reconstructed directly from the immutable V47 update-004 source.

All four candidates receive the complete non-greedy 25-unit/48-row train
...
AUTHORIZATION_ID = "v51_query_alpha_grid"
V51_SCRIPT = Path("src/semantic_3d_chat/evaluation/v51_query_alpha_grid.py")
V51_TEST = Path("tests/test_v51_query_alpha_grid.py")
V50_TERMINAL = Path("reports/gemma4/metrics/v50_scene_query_alpha_terminal_gate.json")
V50_REPORT = Path("reports/gemma4/metrics/v50_scene_query_alpha_grid.json")
DEFAULT_REPORT = Path("reports/gemma4/metrics/v51_query_alpha_grid.json")
DEFAULT_CHECKPOINT_ROOT = Path(
    "data_gemma4/checkpoints/gemma4_v51_query_alpha_grid"
)
...
_V50_REPORT_SHA256 = "158cedd46c73e29fc4cd5e412b6ddd260bb6be187c967c8c4489e5f610cc46f1"
...
_SCENE_ALPHA = 1.0
_QUERY_ALPHAS = (1.75, 2.25, 1.5, 2.5)
_NONSELECTABLE_QUERY_ALPHA_ANCHOR = 2.0
_SCENE_LR = 1.0e-5
_QUERY_LR = 8.0e-6
_EXPECTED_ORIGINAL_PREFIX_RMS = 0.0016845178324729204
_EXPECTED_ORIGINAL_PREFIX_RMS_ABS_TOLERANCE = 1.0e-12
_V50_ANCHOR_FULL_SHA256 = "f1702c1b1a9916de7125476b1a67fd120f0ca81616444e90bd5e656480ef69f2"
_V50_ANCHOR_AUTHORIZED_SHA256 = (
    "5009ee3fcbd0b44c7811938965fe62ae3c447480934cf4515f44ac93de9b9729"
)
...
CANDIDATE_GRID = tuple(
    {
        "candidate_id": (
            f"guarded_scene_alpha_1p0_query_alpha_{_alpha_id(query_alpha)}"
        ),
        "declared_order": index,
        "scene_alpha": _SCENE_ALPHA,
        "query_alpha": query_alpha,
    }
    for index, query_alpha in enumerate(_QUERY_ALPHAS)
)
...
def _validate_terminal_authorization(
    ...
    invocation = _mapping(authorization.get("invocation_contract"), "V51 invocation")
    integrity = _mapping(authorization.get("implementation_integrity"), "V51 integrity")
    source = _mapping(authorization.get("source"), "V51 source")
    u4 = _mapping(source.get("v47_u4"), "V51 V47-u4 source")
    prefix = _mapping(source.get("original_v46_candidate_prefix_reference"), "V51 prefix")
    anchor = _mapping(source.get("v50_query_alpha_two_anchor"), "V51 V50 anchor")
    grid = _mapping(authorization.get("candidate_grid"), "V51 grid")
    evaluation = _mapping(
        authorization.get("evaluation_and_selection"), "V51 evaluation"
    )
    gate = _mapping(authorization.get("per_candidate_gate"), "V51 train gate")
    persistence = _mapping(authorization.get("conditional_persistence"), "V51 persistence")
    scope = _mapping(authorization.get("scope"), "V51 scope")
    checks = {
        "artifact": report.get("artifact") == "v50_scene_query_alpha_terminal_gate",
        ...
        == "one_fixed_v51_train_only_query_alpha_grid",
        "script": authorization.get("authorized_script") == str(V51_SCRIPT),
        "test": authorization.get("authorized_test") == str(V51_TEST),
        ...
        "terminal_path": invocation.get("terminal_path") == str(V50_TERMINAL),
        "cli": invocation.get("required_cli_argument")
        == "--expected-v50-terminal-sha256",
        "no_embedded_sha": invocation.get("v51_must_not_embed_terminal_sha256") is True,
        "terminal_auth": invocation.get(
            "v51_must_authenticate_terminal_bytes_and_exact_authorization"
        )
        is True,
        "script_hash": integrity.get("script_sha256") == _sha256(_resolve(V51_SCRIPT)),
        "test_hash": integrity.get("test_sha256") == _sha256(_resolve(V51_TEST)),
        ...
        "v50_anchor": anchor.get("path") == str(V50_REPORT)
        and anchor.get("sha256") == _V50_REPORT_SHA256
        and anchor.get("candidate_id") == "guarded_scene_alpha_1p0_query_alpha_2p0"
        and anchor.get("scene_alpha") == _SCENE_ALPHA
        and anchor.get("query_alpha") == _NONSELECTABLE_QUERY_ALPHA_ANCHOR
        and anchor.get("nonselectable") is True
        and anchor.get("only_failed_check") == "teacher_positive_sides_at_least_35"
        and anchor.get("positive_sides") == 34
        and anchor.get("complete_units") == 10
        and anchor.get("cross_prefix_complete_units") == 19
        and anchor.get("complete_physical_pair_coverage") == 5
        and anchor.get("broad_nll") == 2.9192593644062677
        and anchor.get("original_prefix_trust_rms") == _EXPECTED_ORIGINAL_PREFIX_RMS
        and anchor.get("greedy_executed") is False
        and anchor.get("checkpoint_written") is False
        and anchor.get("source_restored_exact") is True
        and anchor.get("access_audit_passed") is True,
        "grid_fixed": grid.get("fixed_before_any_candidate_forward") is True
        and grid.get("candidate_count") == 4
        and grid.get("direction_id") == _DIRECTION_ID
        and list(_sequence(grid.get("direction_components"), "V51 components"))
        == list(_DIRECTION_COMPONENTS),
        "gradient_specs": list(
            _sequence(grid.get("isolated_side_gradient_specs"), "V51 gradient specs")
        )
        ...
        "grid_inventory": list(
            _sequence(grid.get("candidates_declared_order"), "V51 candidates")
        )
        == [dict(value) for value in CANDIDATE_GRID],
        "grid_order": grid.get("scene_alpha_fixed") == _SCENE_ALPHA
        and list(
            _sequence(grid.get("query_alpha_grid_declared_order"), "V51 query alphas")
        )
        == list(_QUERY_ALPHAS)
        and grid.get("query_alpha_two_anchor_nonselectable") is True,
        "grid_prefix_invariance": grid.get("all_candidate_scene_tensors_bit_identical")
        is True
        and grid.get("all_candidate_scene_prefixes_bit_identical") is True
        and grid.get("expected_original_prefix_trust_rms")
        == _EXPECTED_ORIGINAL_PREFIX_RMS
        and grid.get("prefix_rms_absolute_tolerance")
        == _EXPECTED_ORIGINAL_PREFIX_RMS_ABS_TOLERANCE,
        ...
        "grid_integrity": grid.get("reconstruct_each_candidate_directly_from_v47_u4")
        is True
        and grid.get("candidate_hash_inventory_fixed_before_any_candidate_forward") is True
        ...
        is True
        and evaluation.get(
            "all_candidates_receive_full_25_unit_teacher_metrics_and_per_unit_nll"
        )
        is True
        ...
        and evaluation.get(
            "every_pre_gate_passing_candidate_receives_full_changed_25_greedy"
        )
        is True
        and evaluation.get("every_pre_gate_passing_candidate_receives_full_broad_48_greedy")
        is True
        and evaluation.get(
            "winner_eligibility_requires_every_non_greedy_and_greedy_check"
        )
        is True
        ...
        "check_names": list(
            _sequence(gate.get("non_greedy_check_names"), "V51 non-greedy names")
        )
        == list(_NON_GREEDY_CHECK_NAMES)
        and list(_sequence(gate.get("greedy_check_names"), "V51 greedy names"))
        ...
        and scope.get("fixed_four_candidate_query_grid") is True
    ...
    if not all(checks.values()):
        raise ValueError(f"V51 terminal authorization changed: {checks}")
    return checks



def require_terminal(
    expected_sha256: str, path: str | Path = V50_TERMINAL
) -> dict[str, Any]:
    if not isinstance(expected_sha256, str) or _HEX64.fullmatch(expected_sha256) is None:
        raise ValueError("V51 expected V50 terminal SHA256 must be lowercase hexadecimal")
    terminal_path = _resolve(path)
    if terminal_path != _resolve(V50_TERMINAL):
        raise ValueError("V51 terminal path is pinned")
    if terminal_path.is_symlink() or not terminal_path.is_file():
        raise FileNotFoundError("V51 exact V50 terminal is unavailable or unsafe")
    payload = terminal_path.read_bytes()
    observed = _sha256_bytes(payload)
    if observed != expected_sha256:
        raise ValueError("V51 V50 terminal differs from explicit invocation SHA256")
    report = _mapping(json.loads(payload), "V50 terminal")
    authorization = _mapping(
        report.get("conditional_successor_authorization"), "V51 authorization"
    )
    checks = _validate_terminal_authorization(report, authorization)
    return {
        "path": str(V50_TERMINAL),
        "sha256": observed,
...
def candidate_from_split_alphas(
    ...
    *,
    ...
    if scene_alpha != _SCENE_ALPHA or query_alpha not in _QUERY_ALPHAS:
        raise ValueError("V51 candidate alpha is outside the fixed grid")
    v48._validate_surface_tensors(source, field="V51 source")
    v48._validate_surface_tensors(direction, field="V51 guarded direction")
    ...
    v48._validate_surface_tensors(result, field="V51 candidate")
    return result
...
def non_greedy_pre_gate_checks(
    ...
    pair = _mapping(evidence.get("pair_metrics"), "V51 pair metrics")
    families = _mapping(pair.get("complete_units_by_family"), "V51 families")
    cross = _mapping(pair.get("cross_prefix_complete_units_by_family"), "V51 cross families")
    retention = _mapping(evidence.get("retention_diagnostics"), "V51 retention")
...
class RealGridBackend:
    ...
    def __init__(self, terminal: Mapping[str, Any], paths: GridPaths) -> None:
        ...
        self._candidate_prefix_hashes: Mapping[str, str] | None = None
    ...
    def _source_attestation(self) -> dict[str, Any]:
        ...
        if result["passed"] is not True:
            raise RuntimeError("V51 exact V47-u4 source attestation failed")
        return result
    ...
    def authenticate_and_prepare(self) -> Mapping[str, Any]:
        ...
        if self._prepared:
            raise RuntimeError("V51 backend preparation may run only once")
        ...
        if tensor_state_sha256(live_v49_candidate) != _V49_CANDIDATE_AUTHORIZED_SHA256:
            raise RuntimeError("V51 live authenticated V49 candidate changed")
        self._direction = {
        ...
        if tuple(self._candidates) != tuple(spec["candidate_id"] for spec in CANDIDATE_GRID):
            raise RuntimeError("V51 candidate inventory changed")
        source_full_state = {
        ...
        if tensor_state_sha256(source_full_state) != _SOURCE_FULL_SHA256:
            raise RuntimeError("V51 source tensor inventory changed before candidate prehash")
        v50_anchor = {
            name: value.detach().float().cpu().clone()
            for name, value in self._delegate._source_surface.items()
        }
        with torch.no_grad():
            for index, name in enumerate(_PARAMETER_NAMES):
                alpha = (
                    _SCENE_ALPHA
                    if index == 0
                    else _NONSELECTABLE_QUERY_ALPHA_ANCHOR
                )
                learning_rate = _SCENE_LR if index == 0 else _QUERY_LR
                v50_anchor[name].add_(
                    torch.sign(self._direction[name]),
                    alpha=-float(alpha * learning_rate),
                )
        v50_anchor_full = dict(source_full_state)
        v50_anchor_full.update(v50_anchor)
        v50_anchor_authorized_sha256 = tensor_state_sha256(v50_anchor)
        v50_anchor_full_sha256 = tensor_state_sha256(v50_anchor_full)
        if (
            v50_anchor_authorized_sha256 != _V50_ANCHOR_AUTHORIZED_SHA256
            or v50_anchor_full_sha256 != _V50_ANCHOR_FULL_SHA256
        ):
            raise RuntimeError("V51 guarded sign does not reproduce the V50 query2 anchor")
        for candidate_id, values in self._candidates.items():
            ...
            self._candidate_expected[candidate_id] = {
                ...
                "scene_readout_state_sha256": tensor_state_sha256(
                    {_PARAMETER_NAMES[0]: values[_PARAMETER_NAMES[0]]}
                ),
                "query_state_sha256": tensor_state_sha256(
                    {name: values[name] for name in _PARAMETER_NAMES[1:]}
                ),
                "frozen_state_sha256": _FROZEN_SHA256,
            }
        scene_hashes = {
            str(value["scene_readout_state_sha256"])
            for value in self._candidate_expected.values()
        }
        if len(scene_hashes) != 1:
            raise RuntimeError("V51 query-only candidates changed the scene tensor")
        self.restore_source()
        self._prepared = True
        return {
            ...
            "v49_candidate_reconstructed_before_v51_direction": (
            ...
            "query_alpha_grid_declared_order": list(_QUERY_ALPHAS),
            "nonselectable_v50_query_alpha_anchor": _NONSELECTABLE_QUERY_ALPHA_ANCHOR,
            "nonselectable_v50_anchor": {
                "candidate_id": "guarded_scene_alpha_1p0_query_alpha_2p0",
                "scene_alpha": _SCENE_ALPHA,
                "query_alpha": _NONSELECTABLE_QUERY_ALPHA_ANCHOR,
                "full_tensor_state_sha256": v50_anchor_full_sha256,
                "authorized_surface_state_sha256": v50_anchor_authorized_sha256,
                "selectable": False,
                "evaluated_as_v51_candidate": False,
            },
            "all_candidate_scene_tensors_bit_identical": len(scene_hashes) == 1,
            "candidate_scene_readout_state_sha256": next(iter(scene_hashes)),
            "expected_original_prefix_trust_rms": _EXPECTED_ORIGINAL_PREFIX_RMS,
    ...
    def reconstruct_candidate(self, candidate: Mapping[str, Any]) -> Mapping[str, Any]:
        ...
        if not self._prepared:
            raise RuntimeError("V51 candidate reconstruction requires preparation")
        ...
        if index < 0 or index >= len(CANDIDATE_GRID) or dict(candidate) != dict(
            ...
            raise ValueError("V51 candidate is outside the exact declared grid")
        ...
        state = {
            ...
            "scene_readout_state_sha256": expected["scene_readout_state_sha256"],
            "query_state_sha256": expected["query_state_sha256"],
        ...
        if previous is not None and (
            ...
            raise RuntimeError("V51 candidate reconstruction is not deterministic")
    ...
    def evaluate_non_greedy(self, candidate_id: str) -> Mapping[str, Any]:
        ...
        if self._active_candidate_id != candidate_id:
            raise RuntimeError("V51 non-greedy evaluation candidate is not live")
        with torch.inference_mode():
            ...
            current_prefixes = {
                scene_id: current_scene_tokens(
                    self._delegate._caches[scene_id],
                    self._delegate._block_core,
                    device=self._delegate._bundle.language.device,
                )
                .detach()
                .cpu()
                .clone()
                for scene_id in sorted(self._delegate._caches)
            }
        prefix_hashes = {
            scene_id: tensor_state_sha256({"scene_tokens": value})
            for scene_id, value in current_prefixes.items()
        }
        if len(prefix_hashes) != 16:
            raise RuntimeError("V51 candidate prefix inventory is not exactly 16 scenes")
        observed_rms = float(rms.detach().cpu())
        if not math.isclose(
            observed_rms,
            _EXPECTED_ORIGINAL_PREFIX_RMS,
            rel_tol=0.0,
            abs_tol=_EXPECTED_ORIGINAL_PREFIX_RMS_ABS_TOLERANCE,
        ):
            raise RuntimeError(
                "V51 fixed scene-alpha prefix RMS differs from the V50 anchor: "
                f"{observed_rms}"
            )
        if self._candidate_prefix_hashes is None:
            self._candidate_prefix_hashes = dict(prefix_hashes)
        elif prefix_hashes != self._candidate_prefix_hashes:
            raise RuntimeError("V51 query-only candidates changed the scene prefix")
        ...
        return {
            ...
            "original_v46_candidate_relative_prefix_trust_rms": observed_rms,
            "candidate_prefix_sha256_by_train_scene": prefix_hashes,
            "candidate_prefix_hash_inventory_sha256": _canonical_sha256(prefix_hashes),
            "candidate_prefix_matches_all_prior_query_alphas": True,
            "candidate_prefix_scene_count": len(prefix_hashes),
        }
    ...
    def evaluate_greedy(self, candidate_id: str) -> Mapping[str, Any]:
        ...
        if self._active_candidate_id != candidate_id:
            raise RuntimeError("V51 greedy evaluation candidate is not live")
    ...
    def stage_checkpoint(
        ...
        if expected is None:
            raise RuntimeError("V51 cannot stage an unevaluated candidate")
        self.restore_source()
        ...
        try:
            ...
            if state != {
                ...
                raise RuntimeError("V51 staged winner tensor state changed")
            runtime = _mapping(
                json.loads(
                    (directory / "runtime_metadata.json").read_text(encoding="utf-8")
                ),
                "V51 staged runtime metadata",
            )
            validate_runtime_checkpoint_metadata(runtime)
            if runtime != runtime_checkpoint_metadata(metadata):
                raise RuntimeError("V51 runtime metadata is not exact sanitization")
            return {
                **state,
            ...
            if restoration.get("passed") is not True:
                raise RuntimeError("V51 failed to restore source after checkpoint staging")
...
def _staged_checkpoint_inventory(directory: Path) -> dict[str, Any]:
    ...
    if observed != expected:
        raise ValueError(f"V51 staged checkpoint inventory changed: {observed}")
    hashes = {}
    for name in expected:
        path = directory / name
        if path.is_symlink() or not path.is_file():
            raise ValueError(f"V51 staged checkpoint file is unsafe: {name}")
...
def _concise_non_greedy(evidence: Mapping[str, Any]) -> dict[str, Any]:
    result = v49._concise_non_greedy(evidence)
    prefix_hashes = _mapping(
        evidence.get("candidate_prefix_sha256_by_train_scene"),
        "V51 candidate prefix hashes",
    )
    if tuple(sorted(prefix_hashes)) != tuple(sorted(_TRAIN_SCENE_IDS)) or any(
        not isinstance(scene_id, str)
        or not isinstance(digest, str)
        or _HEX64.fullmatch(digest) is None
        for scene_id, digest in prefix_hashes.items()
    ):
        raise ValueError("V51 candidate prefix hashes must cover exactly 16 train scenes")
    observed_rms = _finite(
        evidence.get("original_v46_candidate_relative_prefix_trust_rms"),
        "V51 original prefix RMS",
    )
    result.update(
        {
            "candidate_prefix_sha256_by_train_scene": dict(prefix_hashes),
            "candidate_prefix_hash_inventory_sha256": _canonical_sha256(prefix_hashes),
            "candidate_prefix_scene_count": len(prefix_hashes),
            "candidate_prefix_matches_all_prior_query_alphas": evidence.get(
                "candidate_prefix_matches_all_prior_query_alphas"
            )
            is True,
            "matches_v50_scene1_query2_prefix_rms": math.isclose(
                observed_rms,
                _EXPECTED_ORIGINAL_PREFIX_RMS,
                rel_tol=0.0,
                abs_tol=_EXPECTED_ORIGINAL_PREFIX_RMS_ABS_TOLERANCE,
...
def _query_only_invariance(candidates: Sequence[Mapping[str, Any]]) -> dict[str, Any]:
    """Prove query-alpha candidates share one scene tensor and one scene prefix."""

    scene_hashes: list[str] = []
    prefix_inventory_hashes: list[str] = []
    prefix_maps: list[Mapping[str, Any]] = []
    prefix_rms: list[float] = []
    for value in candidates:
        row = _mapping(value, "V51 candidate row")
        reconstruction = _mapping(
            row.get("candidate_reconstruction"), "V51 candidate reconstruction"
        )
        gate = _mapping(row.get("non_greedy_pre_gate"), "V51 non-greedy gate")
        evidence = gate.get("evidence")
        if not isinstance(evidence, Mapping):
            continue
        scene_hashes.append(str(reconstruction.get("scene_readout_state_sha256", "")))
        prefix_inventory_hashes.append(
            str(evidence.get("candidate_prefix_hash_inventory_sha256", ""))
        )
        prefix_maps.append(
            _mapping(
                evidence.get("candidate_prefix_sha256_by_train_scene"),
        ...
        prefix_rms.append(
            _finite(
                evidence.get("original_v46_candidate_relative_prefix_trust_rms"),
                "V51 candidate prefix RMS",
            )
        )
    complete = len(scene_hashes) == len(CANDIDATE_GRID)
    scene_identical = complete and len(set(scene_hashes)) == 1 and bool(scene_hashes[0])
    prefix_identical = (
        complete
        and len(set(prefix_inventory_hashes)) == 1
        and bool(prefix_inventory_hashes[0])
        and all(dict(value) == dict(prefix_maps[0]) for value in prefix_maps[1:])
    )
    rms_matches_anchor = complete and all(
        math.isclose(
            value,
            _EXPECTED_ORIGINAL_PREFIX_RMS,
            rel_tol=0.0,
        ...
        for value in prefix_rms
    )
    return {
        "evaluated_candidate_count": len(scene_hashes),
        "expected_candidate_count": len(CANDIDATE_GRID),
        "scene_readout_state_sha256": scene_hashes[0] if scene_identical else None,
        "candidate_prefix_hash_inventory_sha256": (
            prefix_inventory_hashes[0] if prefix_identical else None
        ),
        "candidate_prefix_sha256_by_train_scene": (
            dict(prefix_maps[0]) if prefix_identical else None
        ),
        "original_v46_candidate_relative_prefix_trust_rms": (
            prefix_rms[0] if rms_matches_anchor else None
        ),
        "expected_v50_scene1_query2_prefix_trust_rms": _EXPECTED_ORIGINAL_PREFIX_RMS,
        "rms_absolute_tolerance": _EXPECTED_ORIGINAL_PREFIX_RMS_ABS_TOLERANCE,
        "all_candidate_scene_tensors_bit_identical": scene_identical,
        "all_candidate_scene_prefixes_bit_identical": prefix_identical,
        "all_candidate_prefix_rms_matches_v50_anchor": rms_matches_anchor,
        "passed": scene_identical and prefix_identical and rms_matches_anchor,
    }
...
def execute_grid_gate(
    *, terminal: Mapping[str, Any], backend: GridBackend, checkpoint_path: Path
    ...
    try:
        ...
        for spec in CANDIDATE_GRID:
            ...
            if pre_passed is not greedy_executed and error is None:
                raise RuntimeError("V51 per-candidate greedy iff-pre-gate contract violated")
            row = {
                "candidate": dict(spec),
    ...
    candidate_evaluation_complete = grid_complete and all(
        row["non_greedy_pre_gate"]["evaluated"]
        and row["source_restoration"].get("passed") is True
        and row["evaluation_error"] is None
        for row in candidates
    )
    query_only_invariance = _query_only_invariance(candidates)
    evaluation_complete = bool(
        candidate_evaluation_complete and query_only_invariance["passed"] is True
    )
            ...
            execution_errors.append(
                {
                    ...
                    "message": "V51 checkpoint destination appeared before atomic publish",
                }
            )
            final_passed = False
        else:
    ...
    if checkpoint_written is not final_passed:
        raise RuntimeError("V51 checkpoint persistence violated iff-final-pass contract")

    return {
        "schema_version": 1,
        "artifact": "v51_query_alpha_grid",
        "passed": final_passed,
        "authorization": {
            "terminal_path": terminal["path"],
            "terminal_sha256": terminal["sha256"],
        ...
        "query_only_scene_prefix_invariance": query_only_invariance,
        "selection": {
        ...
        "final_train_gate": {
            ...
            "access_audit_passed": access_scope_exact,
...
def preflight(
    *, expected_v50_terminal_sha256: str, paths: GridPaths | None = None
) -> dict[str, Any]:
    """Authenticate V51 without loading Gemma, QA records, or scene maps."""
    ...
    expected = GridPaths(
        terminal=_resolve(V50_TERMINAL),
        report=_resolve(DEFAULT_REPORT),
        checkpoint_root=_resolve(DEFAULT_CHECKPOINT_ROOT),
        config=_resolve(DEFAULT_CONFIG),
    )
    if resolved != expected:
        raise ValueError("V51 preflight paths are pinned")
    checkpoint = resolved.checkpoint_root / "update_000"
    if resolved.report.is_symlink() or resolved.report.exists():
        raise FileExistsError("V51 report is one-shot and already exists")
    if resolved.checkpoint_root.is_symlink() or resolved.checkpoint_root.exists():
        raise FileExistsError("V51 checkpoint root must be absent")
    _locked_hash(resolved.config, _CONFIG_SHA256, "V51 config")
    terminal = require_terminal(expected_v50_terminal_sha256, selected.terminal)
    _locked_hash(_resolve(V50_REPORT), _V50_REPORT_SHA256, "V51 V50 report")
    _locked_hash(_resolve(PROTECTED_REPORT), _PROTECTED_REPORT_SHA256, "V51 protected report")
    source = _resolve(SOURCE_CHECKPOINT)
    if source.is_symlink() or not source.is_dir():
        raise FileNotFoundError("V51 source checkpoint is unavailable")
    source_inventory = sorted(path.name for path in source.iterdir())
    if source_inventory != sorted(_SOURCE_FILES):
        raise ValueError("V51 source checkpoint inventory changed")
    readable_source = {
        name: digest for name, digest in _SOURCE_FILES.items() if name != "optimizer.pt"
    }
    for name, digest in readable_source.items():
        _locked_hash(source / name, digest, f"V51 source {name}")
    prefix = _resolve(PREFIX_REFERENCE_CHECKPOINT)
    if prefix.is_symlink() or not prefix.is_dir():
        raise FileNotFoundError("V51 prefix reference checkpoint is unavailable")
    prefix_inventory = sorted(path.name for path in prefix.iterdir())
    if prefix_inventory != sorted(_PREFIX_REFERENCE_FILES):
        raise ValueError("V51 prefix reference checkpoint inventory changed")
    for name, digest in _PREFIX_REFERENCE_FILES.items():
        _locked_hash(prefix / name, digest, f"V51 prefix reference {name}")
    return {
        "schema_version": 1,
        "artifact": "v51_query_alpha_grid_preflight",
        "passed": True,
        "terminal": {
            "path": terminal["path"],
...
def run_grid(
    *,
    expected_v50_terminal_sha256: str,
    paths: GridPaths | None = None,
    backend_factory: Callable[[Mapping[str, Any], GridPaths], GridBackend] | None = None,
) -> dict[str, Any]:
    """Run the exact authenticated V51 grid and atomically emit its report."""
    ...
    if resolved != expected:
        raise ValueError("V51 terminal, report, checkpoint, and config paths are pinned")
    if resolved.report.is_symlink() or resolved.report.exists():
        raise FileExistsError("V51 report is one-shot and will not be overwritten")
    if resolved.checkpoint_root.is_symlink() or resolved.checkpoint_root.exists():
        raise FileExistsError("V51 checkpoint root must be absent")
    _locked_hash(resolved.config, _CONFIG_SHA256, "V51 config")
    terminal = require_terminal(expected_v50_terminal_sha256, selected.terminal)
...
def main() -> int:
    ...
    parser.add_argument("--expected-v50-terminal-sha256", required=True)
    parser.add_argument("--preflight", action="store_true")
    parser.add_argument("--terminal", type=Path, default=V50_TERMINAL)
    ...
    if args.preflight:
        result = preflight(
            expected_v50_terminal_sha256=args.expected_v50_terminal_sha256,
            paths=paths,
        )
        summary = {
