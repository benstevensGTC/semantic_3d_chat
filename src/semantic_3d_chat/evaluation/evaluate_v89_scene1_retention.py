"""Evaluate V89's one fixed-final retention-aware scene-one checkpoint.
...
PREDICTIONS_ARTIFACT: Final[str] = "gemma4_v89_scene1_retention_predictions_v1"
EVALUATION_ARTIFACT: Final[str] = "gemma4_v89_scene1_retention_evaluation_v1"
_FORBIDDEN_COMPONENTS: Final[frozenset[str]] = frozenset(
    {"oracle", "validation", "test", "deferred"}
)


def _canonical_causal_rows_v89(rows: Sequence[RowV73]) -> tuple[RowV73, ...]:
    ...
    if any((row.question, row.answer) != expected[row.question_id] for row in selected):
        raise ValueError("V89 canonical causal reference changed")
    return selected


def _smoke_rows_v89(config: Mapping[str, Any]) -> tuple[RowV73, ...]:
    answer_types = ("presence", "attribute", "spatial_relation")
    return tuple(
        RowV73(
            scene_id=SCENE_ID,
            question_id=f"v89_smoke_{ordinal:02d}",
            question=str(raw["question"]),
            answer=normalize_answer(raw["expected"]),
            answer_class=f"v89_smoke_class_{ordinal:02d}",
            answer_type=answer_type,
            pair_id="v89_development_known_smoke",
            paired_scene_id=SCENE_ID,
            question_key=f"v89_smoke_{ordinal:02d}",
            change_type="none",
...
def run_evaluation_v89(config_path: str | Path = CONFIG) -> dict[str, Any]:
    started = time.monotonic()
    config = load_config_v89(config_path)
    source_hashes = authenticate_sources_v89(config)
    training_bindings = authenticate_training_report_v89(config, config_path=config_path)
    rows = load_canonical_rows_v89(config)
    causal_rows = _canonical_causal_rows_v89(rows)
    smoke_rows = _smoke_rows_v89(config)
    ...
    if predictions_path.exists() or report_path.exists():
        raise FileExistsError("V89 create-once evaluation output exists")
    ...
    with audit:
        ...
        if language.device.type != "mps":
            raise RuntimeError("V89 fixed-final evaluation requires local MPS")
        ...
        if not isinstance(collection, LoRABankCollection):
            raise TypeError("V89 evaluation LoRA installation failed")
        ...
        candidate = load_fixed_final_bridge_v89(
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    report = run_evaluation_v89(args.config)
    print(
        json.dumps(
            {
