"""Select a fully completed V40 cross-preserving development checkpoint.

This is a deliberately fail-closed post-training process.  The exact V40
...
from semantic_3d_chat.training.train_cross_preserving_v40 import (
    OPTIMIZER_AUDIT_FILENAME,
    V40Contract,
    optimizer_step_audit,
    replay_v40_gates,
    require_exact_v40_sources,
    require_v39_terminal_gate,
    retag_bundle_for_v40,
    v40_contract,
    v40_loader_config,
...
DEFAULT_OUTPUT = Path("reports/gemma4/metrics/v40_cross_preserving_selection.json")
...
def _query_state(tensors: Mapping[str, torch.Tensor]) -> dict[str, torch.Tensor]:
    ...
    if observed != _QUERY_PARAMETER_NAMES:
        raise ValueError("V40 query-bank tensor order or inventory changed")
    if tuple(tuple(value.shape) for value in state.values()) != _QUERY_SHAPES:
        raise ValueError("V40 query-bank tensor shapes changed")
    return state
...
def install_v40_target_b(
    ...
    if tuple(query) != ("lora_b",) or tuple(query["lora_b"].shape) != _QUERY_SHAPES[0]:
        raise ValueError("V40 target installer received a changed LoRA-B state")
    ...
    if not isinstance(lora_a, torch.Tensor) or not isinstance(lora_b, torch.Tensor):
        raise TypeError("V40 target adapter lacks LoRA A/B tensors")
    ...
    if a_before != a_after or b_after != expected_b:
        raise RuntimeError("V40 target installation changed LoRA-A or failed to install B")
    return {
...
def _checkpoint_paths_or_raise(
    checkpoint_root: Path, contract: V40Contract
) -> tuple[Path, ...]:
    ...
    if checkpoint_root.resolve() != _AUTHORIZED_CHECKPOINT_ROOT:
        raise ValueError("V40 selector checkpoint root differs from terminal authorization")
    if checkpoint_root.is_symlink() or not checkpoint_root.is_dir():
        raise ValueError("V40 checkpoint root must be a real directory")
    if tuple(contract.saved_optimizer_steps) != _EXPECTED_STEPS:
        raise ValueError("V40 saved-step contract changed")
    checkpoints = tuple(
    ...
    if observed != expected:
        raise FileNotFoundError(
            "V40 requires the exact completed update-41 envelope: "
            f"observed={observed} expected={expected}"
        )
    for step, checkpoint in zip(contract.saved_optimizer_steps, checkpoints, strict=True):
        if checkpoint.is_symlink() or not checkpoint.is_dir():
            raise ValueError(f"V40 arm must be a real directory: {checkpoint}")
        required = [
            "adapter.safetensors",
            TRAINING_METADATA_FILENAME,
            RUNTIME_METADATA_FILENAME,
        ...
        if any(
            ...
            raise FileNotFoundError(f"V40 arm is incomplete or aliased: {checkpoint.name}")
        if step == 0 and any(
            (checkpoint / name).exists()
            for name in ("optimizer.pt", OPTIMIZER_AUDIT_FILENAME)
        ):
            raise ValueError("V40 update zero must not persist SGD state")
    return checkpoints
...
def _validate_train_only_boundary(
    ...
    if any(stage.get(key) is not False for key in required_false):
        raise ValueError(f"V40 {label} crossed its train-only boundary")
    if stage.get("deferred_final_scene_ids_loaded") != []:
        raise ValueError(f"V40 {label} touched deferred final scenes")
    ...
    if (
        ...
        raise ValueError(f"V40 {label} QA provenance is not exact train-only input")
    ...
    try:
        ...
        raise ValueError(f"V40 {label} map provenance is not exact train-only input") from exc
    if (
        ...
        raise ValueError(f"V40 {label} map provenance is not exact train-only input")
...
def _validate_prefix_replay(
    ...
    if (
        ...
        raise ValueError(f"V40 {label} prefix replay is incomplete or non-deterministic")
...
def _validate_surface(
    ...
    if dict(surface) != expected_surface:
        raise ValueError("V40 persisted trainable surface changed")
    ...
    bank_hashes = _mapping(metadata.get("lora_bank_state_sha256"), "V40 LoRA hashes")
    if (
        ...
        raise ValueError("V40 query-bank state or metadata changed")
    if (
        frozen_hash != contract.frozen_state_sha256
        or frozen_hash != stage.get("frozen_excluding_query_state_sha256")
    ):
        raise ValueError("V40 changed a frozen tensor or buffer")
    if (
        ...
        raise ValueError("V40 changed its frozen K-only V23 hybrid")
    if (
        ...
        raise ValueError("V40 changed its frozen learned block core")
    if metadata.get("lora_trainable_parameter_count") != 0:
        raise ValueError("V40 checkpoint advertises the wrong trainable count")
    return {
...
def _row_at(history: list[Mapping[str, Any]], step: int) -> Mapping[str, Any]:
    if step >= len(history) or history[step].get("optimizer_update") != step:
        raise ValueError(f"V40 history lacks exact optimizer update {step}")
    return history[step]
...
def _validate_update_zero_evidence(
    ...
    attestation = _mapping(stage.get("update_zero_attestation"), "V40 update-zero proof")
    required = {
    ...
    if any(attestation.get(key) != value for key, value in required.items()):
        raise ValueError("V40 update-zero hybrid attestation changed")
    baseline = _mapping(attestation.get("behavioral_baseline"), "V40 behavioral baseline")
    observed = _mapping(baseline.get("observed"), "V40 update-zero observed baseline")
    expected = _mapping(baseline.get("expected"), "V40 update-zero expected baseline")
    ...
    if stage.get("source_audit") != dict(source_audit):
        raise ValueError("V40 exact source audit changed")
    row0 = _row_at(history, 0)
    source_pairs = _mapping(stage.get("source_pair_metrics"), "V40 source pair metrics")
    source_diagnostics = stage.get("source_per_unit_nll_diagnostics")
    if not isinstance(source_diagnostics, list):
        raise TypeError("V40 source per-unit NLL diagnostics are absent")
    ...
    if (
        ...
        raise ValueError("V40 update-zero history/source evidence changed")
    greedy = _mapping(stage.get("source_train_greedy_metrics"), "V40 source greedy")
    if (
        ...
        raise ValueError("V40 source broad-greedy baseline changed")
    return {
        ...
        "source_residual_diagnostics": dict(
            _mapping(stage.get("source_residual_diagnostics"), "V40 source residual")
        ),
    }


def validate_v40_checkpoint_envelope(
    config: Mapping[str, Any],
    checkpoint_root: Path,
    contract: V40Contract,
) -> tuple[tuple[Path, ...], list[dict[str, Any]]]:
    """Authenticate the full V40 run before validation construction is legal."""

    checkpoints = _checkpoint_paths_or_raise(checkpoint_root, contract)
    terminal = require_v39_terminal_gate(config)
    hybrid, _source_metadata, raw_source_audit = require_exact_v40_sources(config)
    source_audit = {
        **raw_source_audit,
        "loader_transition": {
            ...
            "v40_manually_trainable_adapter": (
    ...
    for step, checkpoint in zip(contract.saved_optimizer_steps, checkpoints, strict=True):
        ...
        if metadata.get("optimizer_step") != step or stage.get("optimizer_step") != step:
            raise ValueError(f"V40 optimizer-step metadata changed: {checkpoint.name}")
        if metadata.get("config_hash") != expected_config_hash:
            raise ValueError(f"V40 config hash changed: {checkpoint.name}")
        ...
        if (
            ...
            raise ValueError(f"V40 terminal authorization changed: {checkpoint.name}")
        expected_static = {
        ...
        if any(stage.get(key) != value for key, value in expected_static.items()):
            raise ValueError(f"V40 source or hybrid provenance changed: {checkpoint.name}")
        _validate_train_only_boundary(stage, checkpoint.name, provenance)
        _validate_prefix_replay(stage, checkpoint.name, provenance)
        prefix_replay = _mapping(stage["prefix_replay_attestation"], "V40 prefix replay")
        if common_prefix_replay is None:
            common_prefix_replay = prefix_replay
        elif dict(prefix_replay) != dict(common_prefix_replay):
            raise ValueError("V40 prefix replay changed across checkpoints")
        schedule = _mapping(stage.get("schedule"), "V40 schedule")
        if (
            ...
            raise ValueError(f"V40 schedule proof changed: {checkpoint.name}")
        if common_schedule is None:
            common_schedule = schedule
        elif dict(schedule) != dict(common_schedule):
            raise ValueError("V40 schedule metadata changed across checkpoints")

        raw_history = metadata.get("history")
        if not isinstance(raw_history, list) or len(raw_history) != step + 1:
            raise ValueError(f"V40 history is incomplete: {checkpoint.name}")
        history = [_mapping(row, "V40 history row") for row in raw_history]
        if [row.get("optimizer_update") for row in history] != list(range(step + 1)):
            raise ValueError(f"V40 history is not one row per true update: {checkpoint.name}")
        if prior_history and history[: len(prior_history)] != prior_history:
            raise ValueError("V40 rewrote prior optimizer history in a later checkpoint")
        if any(
            row.get("validation_qa_loaded") is not False
            or row.get("oracle_environment_files_loaded") is not False
            for row in history
        ):
            raise ValueError(f"V40 history crossed its train-only boundary: {checkpoint.name}")
        for row in history[1:]:
            guard = _mapping(row.get("component_gradient_guard"), "V40 gradient guard")
            clip = _mapping(
                row.get("clip_direction_attestation"), "V40 clip attestation"
            )
            if (
                ...
                raise ValueError(
                    f"V40 per-microstep direction/frozen-state proof changed: {checkpoint.name}"
                )
        prior_history = history
        if history[-1].get("saved_checkpoint") is not True:
            raise ValueError(f"V40 saved arm lacks its saved-row proof: {checkpoint.name}")
        ...
        if common_update_zero is None:
            ...
            raise ValueError("V40 deterministic update-zero evidence changed across arms")
        if step in _DIAGNOSTIC_STEPS:
            ...
            if not isinstance(diagnostics, list) or not isinstance(pairs, Mapping):
                raise ValueError(
                    f"V40 update-{step} lacks all 25 persisted per-unit NLL diagnostics"
                )
        ...
        if runtime != runtime_checkpoint_metadata(metadata):
            raise ValueError(f"V40 runtime metadata is not freshly sanitized: {checkpoint.name}")
        ...
        if update0 is None:
            ...
            if (
                ...
                raise ValueError("V40 update zero is not exact authenticated V38 update zero")
            if surface["query_bank_state_sha256"] != contract.query_source_state_sha256:
                raise ValueError("V40 update zero changed inherited layer-14 LoRA-B")
        else:
            if set(tensors) != set(update0):
                raise ValueError(f"V40 tensor inventory changed: {checkpoint.name}")
            if any(
                tuple(tensors[name].shape) != tuple(update0[name].shape)
                or tensors[name].dtype != update0[name].dtype
                for name in tensors
            ):
                raise ValueError(f"V40 tensor shape/dtype changed: {checkpoint.name}")
            ...
            if not changed or not changed.issubset(_QUERY_PARAMETER_NAME_SET):
                raise ValueError(f"V40 changed a frozen or no target tensor: {checkpoint.name}")
            if surface["query_bank_state_sha256"] == contract.query_source_state_sha256:
                raise ValueError(f"V40 learned layer-14 LoRA-B did not transition: {checkpoint.name}")
        ...
        for gate_step, gate in replayed.items():
            if step < gate_step:
                if gate is not None:
                    raise ValueError(f"V40 arm persisted a future update-{gate_step} gate")
                continue
            if (
                gate is None
                ...
                raise ValueError(f"V40 lacks a passed update-{gate_step} train-only gate")
            prior = accepted_gates.get(gate_step)
            if prior is None:
                accepted_gates[gate_step] = gate
            elif dict(gate) != dict(prior):
                raise ValueError(f"V40 update-{gate_step} gate changed across later arms")
        audits.append(
            {
    ...
    if set(accepted_gates) != {8, 16, 41}:
        raise ValueError("V40 completed envelope lacks all three passed train-only gates")
    if (
        source_audit.get("source_optimizer_file_opened") is not False
        or source_audit.get("source_optimizer_state_loaded") is not False
    ):
        raise ValueError("V40 source audit crossed the no-source-optimizer boundary")
    return checkpoints, audits


class V40ArmEvaluator(Protocol):
...
class _V40RuntimeEvaluator(_V35RuntimeEvaluator):
    """One-Gemma evaluator with exact V40 V23/query/core installation."""
    ...
    def __init__(
        ...
        actual = dict(config)
        construction = v40_loader_config(actual)
        super().__init__(construction, control_config, checkpoint, requirements)
        self.loader_transition = retag_bundle_for_v40(self.bundle, actual)
        self.config = actual
        collection = self.bundle.lora_installation
        if collection is None:
            raise RuntimeError("V40 evaluator requires installed LoRA banks")
        self._v40_target_adapter = collection.bank(_QUERY_BANK).installation.adapters[1]
        self._v40_v23 = collection.bank(_V23_BANK).installation.state_module
        self._v40_contract = v40_contract(actual)
    ...
    def install(
        ...
        target_installation = install_v40_target_b(self._v40_target_adapter, query)
        self._v40_v23.load_state_dict(v23, strict=True)
        self._v40_target_adapter.requires_grad_(False).eval()
        self._v40_v23.requires_grad_(False).eval()
        if (
            tensor_state_sha256({"lora_b": self._v40_target_adapter.lora_b})
            ...
            or tensor_state_sha256(self._v40_v23.state_dict()) != tensor_state_sha256(v23)
            ...
            raise RuntimeError("V40 evaluator did not install the supplied tensor envelope")
        if not approved_v29 and (
            tensor_state_sha256(v23) != self._v40_contract.hybrid_v23_state_sha256
            or tensor_state_sha256(core) != self._v40_contract.core_state_sha256
        ):
            raise RuntimeError("V40 evaluator changed its frozen V23/core state")
        if any(
            parameter.requires_grad
            for module in self.bundle.checkpoint_modules.values()
            for parameter in module.parameters()
        ):
            raise RuntimeError("V40 selector evaluation left a checkpoint tensor trainable")

    def evaluate_teacher(self) -> V35TeacherEvidence:
        evidence = super().evaluate_teacher()
        prefix = dict(evidence.prefix_diagnostics)
        prefix["tensor"] = "composed_v40_question_independent_continuous_scene_prefix"
...
def _validate_exact_development_evidence(
    ...
    if len(teacher.pair_margins.unit_keys) != 12 or len(teacher.pair_margins.margins) != 12:
        raise ValueError("V40 teacher evidence must score exactly 12 changed validation units")
    if (
        ...
        raise ValueError("V40 greedy evidence must score exactly 12 changed validation units")
...
def select_v40(
    ...
    *,
    evaluator_factory: Callable[
        [dict[str, Any], dict[str, Any], Path, SelectionRequirements], V40ArmEvaluator
    ] = _V40RuntimeEvaluator,
) -> dict[str, Any]:
    ...
    checkpoints, envelope_audits = validate_v40_checkpoint_envelope(
    ...
    if tuple(evaluator.validation_scene_ids) != expected_validation:
        raise ValueError("V40 evaluator must remain exactly on validation scenes 19--24")
    ...
    for step, checkpoint in zip(contract.saved_optimizer_steps, checkpoints, strict=True):
        ...
        if len(teacher.pair_margins.unit_keys) != 12:
            raise ValueError("V40 teacher scorer omitted a changed validation unit")
        ...
        key=lambda arm: (
            ...
            -sum(
                ...
                for value in _mapping(
                    arm["greedy_complete_units_by_family"], "V40 greedy families"
                ).values()
            ),
    ...
    if selected is not None:
        ...
        if initial.get("passed") is not True:
            raise ValueError("V40 selected prefix failed its pre-question invariance replay")
    ...
    return {
        "schema_version": 1,
        "artifact": "v40_cross_preserving_development_selection",
        ...
        "improvement_baseline": "exact_v40_k_only_hybrid_update_000",
...
def run_v40_selector(config: Path, checkpoint_root: Path, output: Path) -> dict[str, Any]:
    """Write only after selection returns; all refusal paths leave no report."""

    report = select_v40(config, checkpoint_root)
...
def main() -> int:
    ...
    report = run_v40_selector(args.config, args.checkpoint_root, args.output)
...
__all__ = [
    ...
    "run_v40_selector",
    "select_v40",
    "validate_v40_checkpoint_envelope",
]
