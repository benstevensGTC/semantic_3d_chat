"""Evaluate V87's one fixed-final class-balanced scene-one checkpoint.

This fixed scorer measures all 138 training-set rows once, the same three
native-boundary zero-payload controls, and the corrected three-question generic
smoke.  It also enforces preregistered attribute, presence, and spatial-relation
floors.  Passing authorizes only a separate oracle-unavailable runtime package;
this process cannot promote or mutate a checkpoint.
"""
...
from semantic_3d_chat.training.train_v87_scene1_balanced import (
...
PREDICTIONS_ARTIFACT: Final[str] = "gemma4_v87_scene1_balanced_predictions_v1"
EVALUATION_ARTIFACT: Final[str] = "gemma4_v87_scene1_balanced_evaluation_v1"
...
def _smoke_rows(config: Mapping[str, Any]) -> tuple[RowV73, ...]:
    ...
    return tuple(
        RowV73(
            scene_id=SCENE_ID,
            question_id=f"v87_smoke_{ordinal:02d}",
            question=str(raw["question"]),
            answer=normalize_answer(raw["expected"]),
            answer_class=f"v87_smoke_class_{ordinal:02d}",
            answer_type=answer_type,
            pair_id="v87_smoke",
            paired_scene_id=SCENE_ID,
            question_key=f"v87_smoke_{ordinal:02d}",
            change_type="none",
...
def score_records_v87(
    ...
    *,
    ...
    if len(rows) != 138 or len(records) != 138:
        raise ValueError("V87 score requires all 138 scene-one rows")
    row_by_id = {row.question_id: row for row in rows}
    if {str(record["question_id"]) for record in records} != set(row_by_id):
        raise ValueError("V87 prediction inventory differs from fixed references")
    ...
    if len(causal_records) != 3:
        raise ValueError("V87 causal control requires exactly three rows")
    causal_margin = sum(float(record["zero_minus_correct_nll"]) for record in causal_records) / len(
        causal_records
    )
    causal_changes = sum(
    ...
    if len(smoke_records) != 3:
        raise ValueError("V87 generic smoke requires exactly three rows")
    ...
    model_gates = {
        ...
        "attribute_accuracy_at_least_0_50": by_type["attribute"]["accuracy"]
        >= float(gates["attribute_accuracy_minimum"]),
        "presence_accuracy_at_least_0_75": by_type["presence"]["accuracy"]
        >= float(gates["presence_accuracy_minimum"]),
        "spatial_relation_accuracy_at_least_0_60": by_type["spatial_relation"]["accuracy"]
        ...
        "generic_live_smoke_exactly_3_of_3": smoke_correct
        == int(gates["live_smoke_required_correct"]),
        "causal_correct_memory_mean_nll_below_zero_payload": causal_margin > 0.0,
        "causal_prediction_change_at_least_1": causal_changes
...
def run_evaluation_v87(config_path: str | Path = CONFIG) -> dict[str, Any]:
    started = time.monotonic()
    config = load_config_v87(config_path)
    source_hashes = authenticate_sources_v87(config)
    training_bindings = authenticate_training_report_v87(config, config_path=config_path)
    ...
    if predictions_path.exists() or report_path.exists():
        raise FileExistsError("V87 create-once evaluation output exists")
    ...
    with audit:
        ...
        if language.device.type != "mps":
            raise RuntimeError("V87 fixed-final evaluation requires local MPS")
        ...
        if not isinstance(collection, LoRABankCollection):
            raise TypeError("V87 evaluation LoRA installation failed")
        ...
        candidate = load_fixed_final_bridge_v87(
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    report = run_evaluation_v87(args.config)
    print(
        json.dumps(
            {
