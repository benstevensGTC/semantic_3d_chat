"""Label-blind four-arm predictor for V95's fixed known-development gate.

All six complete scene memories and all three causal variants are bound before
the sanitized question manifest is opened.  The process blocks every QA and
oracle root plus direct V94 prediction/score artifacts.  It creates no public
output until all 216 rows have generated successfully.
"""
...
def load_predictor_stack_v95(
    config: Mapping[str, Any], *, expected_candidate_state_sha256: str
) -> PredictorStackV95:
    """Load the exact nine-bank frozen inference stack and fixed V95 bridge."""
    ...
    if not isinstance(collection, LoRABankCollection):
        raise TypeError("V95 known-development LoRA installation failed")
    load_frozen_parent_v95(collection, config)
    candidate = load_fixed_final_bridge_v95(collection, config["outputs"]["fixed_final_candidate"])
    if candidate.get("state_sha256") != expected_candidate_state_sha256:
        raise ValueError("V95 predictor loaded a different fixed-final candidate")
    collection.eval()
    language.model.eval()
    language.decoder_module.eval()
    return PredictorStackV95(
        language=language,
...
def generate_arm_v95(stack: PredictorStackV95, memory: torch.Tensor, question: str) -> str:
...
@hardened_evaluation_stage_v95
def predict_known_development_v95(
    config_path: str | Path = CONFIG,
) -> dict[str, Any]:
    """Generate exact 216-row four-arm predictions without opening labels."""
    ...
    assert_output_bundle_state_v95(_all_outputs(paths), complete=False)
    ...
    with audit:
        ...
        # Security-critical ordering: every primary, zero, permutation, and
        # paired-wrong memory is complete before the first question file read.
        ...
        provenance = prediction_provenance_v95(config_path, fixed, questions)
        rows: list[dict[str, Any]] = []
        for ordinal, question in enumerate(questions.questions, 1):
            scene_id = question.scene_id
            predictions = {
                arm: generate_arm_v95(stack, fixed.memories[arm][scene_id], question.question)
                for arm in ARMS
            }
            current_hashes = {arm: prefix_sha256(fixed.memories[arm][scene_id]) for arm in ARMS}
            rows.append(
                prediction_row_v95(
                    scene_id=scene_id,
                    ...
                    unchanged=all(
                        current_hashes[arm] == fixed.memory_hashes[arm][scene_id] for arm in ARMS
                    ),
                )
            )
            if ordinal == 1 or ordinal % 12 == 0 or ordinal == QUESTION_COUNT:
                print(
                    json.dumps(
                        {
                            "event": "v95_known_development_prediction",
                            "ordinal": ordinal,
    ...
    if hashes_after != fixed.memory_hashes:
        raise RuntimeError("V95 predictor mutated a fixed scene/control memory")
    validate_prediction_rows_v95(
        rows,
        questions=questions,
    ...
    if access["passed"] is not True or access["protected_read_count"] != 0:
        raise RuntimeError("V95 predictor protected-file audit failed")

    # Create-once publication happens only after all generation and checks pass.
    write_json_create_once_v95(paths.provenance, provenance)
    write_jsonl_create_once_v95(paths.predictions, rows)
    write_json_create_once_v95(paths.prediction_access, access)
    completion = {
    ...
    write_json_create_once_v95(paths.prediction_completion, completion)
    bundle = authenticate_prediction_bundle_v95(config_path)
    return {
        **completion,
        "completed": True,
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    print(json.dumps(predict_known_development_v95(args.config), indent=2, sort_keys=True))
    return 0
