"""Strict no-live-step structural preflight for the V19 signed-X branch.

This supervised, offline diagnostic reconstructs the exact first V19 epoch
...
# The V19 curriculum intentionally inherits the already-audited V18 selection
...
class V19StructuralPreflightViolation(ValueError):
    """A fail-closed V19 configuration or evidence violation."""


def _fail(message: str) -> None:
    raise V19StructuralPreflightViolation(message)
...
def _validate_optimizer_contract(raw: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if normalized != pinned:
        _fail(f"V19 AdamW contract mismatch: expected={pinned} observed={normalized}")
    return normalized


def validate_v19_config_contract(config: Mapping[str, Any]) -> dict[str, Any]:
    """Validate the complete gradient-defining V19 launch surface."""
    ...
    if config.get("structural_preflight") is not None or config.get("v18_screen") is not None:
        _fail("V19 must not inherit a completed V18 controller or preflight contract")
    scene_encoder = _mapping(config.get("scene_encoder"), "scene_encoder")
    if scene_encoder.get("global_latents") != 256:
        _fail("V19 requires exactly 256 globally complete scene slots")
    ...
    if (
        ...
        _fail("V19 requires the enabled 128D centered-content V18 residual base")
    if (
    ...
    if failed_training:
        _fail(f"V19 training contract mismatch: {failed_training}")
    ...
    if (
        ...
        _fail("V19 pair curriculum no longer defines the exact twelve-step candidate schedule")
    ...
    if (
        ...
        _fail("V19 opaque per-pair objective policy mismatch")
    ...
    if failed_experiment:
        _fail(f"V19 experiment contract mismatch: {failed_experiment}")

    screen = _mapping(config.get("v19_screen"), "v19_screen")
    expected_screen = {
        "schema_version": 1,
        "role": V19_SCREEN_ROLE,
    ...
    if dict(screen) != expected_screen:
        _fail("V19 staged screen contract mismatch")
    ...
    if failed_language:
        _fail(f"V19 language contract mismatch: {failed_language}")

    normalized = {
        "schema_version": 1,
        "role": V19_PREFLIGHT_ROLE,
        "source_checkpoint_epoch": 4,
        "latent_count": 256,
        "scene_dim": 1536,
        "content_dim": 128,
...
def ordered_curriculum_evidence(curriculum: Sequence[Any]) -> tuple[list[dict[str, Any]], str]:
    """Return the exact opaque identity and hash of the V19 epoch-one order."""

    entries: list[dict[str, Any]] = []
    for microstep, batch in enumerate(curriculum, start=1):
        if getattr(batch, "kind", None) != "pair":
            _fail(f"V19 microstep {microstep} is not a pair batch")
        units = tuple(getattr(batch, "pair_units", ()))
        if len(units) != 1:
            _fail(f"V19 microstep {microstep} must contain exactly one pair unit")
        unit = units[0]
        entries.append(
            {
                "microstep": microstep,
...
def exact_clone_adamw_evidence(
    ...
    if [name for name, _parameter in live_named] != ["output_projection.weight"]:
        raise ValueError("V19 signed-X trainable surface is not exactly the output weight")
    if live_named[0][1].grad is None:
        raise ValueError("V19 signed-X clone simulation requires an accumulated gradient")
...
def functional_signed_x_delta(
    ...
    *,
    ...
    """Evaluate the predicted FP32 centered delta and its effective cast."""
    ...
    effective = base_tokens + raw_delta.to(dtype=base_tokens.dtype)
    return raw_delta, effective - base_tokens


def evaluate_v19_structural_gate(
    ...
    *,
    ...
    maximum_delta_to_core_rms_ratio: float = 0.01,
) -> dict[str, Any]:
    """Apply the predeclared all-slot, centering, and magnitude launch gate."""

    if not raw_scene_metrics or set(raw_scene_metrics) != set(effective_scene_metrics):
        raise ValueError("Raw/effective V19 scene metric sets must be equal and nonempty")
    ...
    for scene_id in sorted(raw_scene_metrics):
        ...
        per_scene[scene_id] = {
            ...
            "raw_delta_ratio_bounded": float(raw["delta_to_core_rms_ratio"])
            <= maximum_delta_to_core_rms_ratio,
    ...
    passed = all_slots and all(all(checks.values()) for checks in per_scene.values())
    return {
        "schema_version": 1,
        "maximum_delta_to_core_rms_ratio": maximum_delta_to_core_rms_ratio,
...
def _zero_output_equivalence(
    *,
    ...
    from semantic_3d_chat.scene_encoder.signed_x_residual import apply_signed_x_scene_residual
...
def run_preflight(config_path: str | Path, report_path: str | Path) -> dict[str, Any]:
    """Execute the real V19 epoch-one diagnostic without a live optimizer step."""
    ...
    if tuple(sorted(by_scene)) != EXPECTED_SCENE_IDS:
        _fail(f"V19 selected scene mismatch: {sorted(by_scene)}")
    ...
    if len(curriculum) != 12 or ordered_hash != EXPECTED_ORDERED_UNIT_SHA256:
        _fail(
            "V19 ordered epoch-one unit mismatch: "
    ...
    if pair_membership_hash != EXPECTED_PAIR_MEMBERSHIP_SHA256:
        _fail("V19 pair-membership hash mismatch")
    ...
    if lora is None or lora.trainable_parameter_count != 0:
        _fail("V19 requires both named LoRA banks installed and entirely frozen")
    lora.eval()
    ...
    if global_residual is None:
        _fail("V19 global residual construction returned None")
    ...
    if signed_residual is None:
        _fail("V19 signed-X residual construction returned None")
    ...
    if initial_signed_hash != EXPECTED_SIGNED_X_INITIAL_STATE_SHA256:
        _fail("V19 deterministic signed-X initial-state mismatch")
    if torch.count_nonzero(signed_residual.output_projection.weight).item() != 0:
        _fail("V19 signed-X output projection is not exact zero at construction")
    ...
    if source_artifact_hashes != {
        ...
        _fail(f"V19 source artifact hash mismatch: {source_artifact_hashes}")
    scene_state_modules = {
    ...
    validate_global_scene_residual_state(
        ...
        context="V19 preflight frozen V18 source",
    )
    validate_signed_x_scene_residual_state(
        signed_residual,
        expected_parameter_count=196_608,
        context="V19 preflight fresh signed-X branch",
    )
    ...
    if observed_source_hashes != expected_source_hashes:
        _fail(
            "V19 loaded source state hash mismatch: "
    ...
    if structural_state.get("parameter_count") != 196_608:
        _fail("V19 signed-X structural parameter count mismatch")
    ...
    if not zero_equivalence["verified"]:
        _fail("V19 fresh signed-X branch failed exact step-zero identity")
    ...
    for microstep, batch in enumerate(curriculum, start=1):
        ...
        if not isinstance(margins, torch.Tensor):
            _fail("V19 pair objective did not return differentiable candidate margins")
        ...
        if not isinstance(full_vocab_loss, torch.Tensor) or not isinstance(
            ...
            _fail("V19 pair objective did not return full-vocabulary ranking tensors")
        ...
        if output_parameter.grad is None:
            _fail(f"V19 microstep {microstep} produced no signed-X gradient tensor")
        ...
        if unit.pair_id == COLOR_PAIR_ID:
            ...
            _fail(f"Unexpected V19 opaque pair ID: {unit.pair_id}")
    ...
    if final_gradient is None:
        _fail("V19 accumulated gradient disappeared")
    ...
    structural_gate = evaluate_v19_structural_gate(
        raw_scene_metrics,
        effective_scene_metrics,
        structural_state=structural_state,
        maximum_delta_to_core_rms_ratio=contract["maximum_delta_to_core_rms_ratio"],
    )
    ...
    authorization_checks = {
        ...
        "fp32_centered_all_slot_delta_gate": bool(structural_gate["passed"]),
    ...
    signed_implementation_path = Path(
        __import__(
            "semantic_3d_chat.scene_encoder.signed_x_residual",
            fromlist=["__file__"],
        ).__file__
    ).resolve()
    frozen_state_hashes = {
    ...
    if not authorization:
        raise V19StructuralPreflightViolation(
            f"V19 structural preflight failed; evidence written to {destination}"
        )
    print(json.dumps({"phase": "v19_structural_preflight_passed", "report": str(destination)}))
    return report
