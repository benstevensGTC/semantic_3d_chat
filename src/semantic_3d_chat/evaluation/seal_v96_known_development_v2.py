"""Label/model-free final gate sealer for V96 evaluator-auth revision v2."""
...
from semantic_3d_chat.evaluation.v96_evaluation_io_v2 import physical_path_v96_v2
...
def _build_final_reports_v96(
    *,
    ...
    attestation_immutable = (
        current["attestation_file_sha256"]
        == candidate["attestation_file_sha256"]
        == structured["report"]["candidate_attestation_file_sha256"]
        == nll["report"]["candidate_attestation_file_sha256"]
    )
    ...
    gates = known_development_gate_results_v96(
        ...
        immutable_fixed_final=immutable and attestation_immutable,
    ...
    shared = {
        ...
        "model_snapshot_inventory_sha256": candidate[
            "model_snapshot_inventory_sha256"
        ],
        "weights_hashed_not_model_loaded": candidate[
        ...
        "candidate_attestation_immutable": attestation_immutable,
        "frozen_v95_parent_immutable": frozen_parent_immutable,
...
def seal_known_development_v96(
    ...
    final_path = physical_path_v96_v2(paths.final_score)
    evidence_path = physical_path_v96_v2(paths.evidence)
    states = [
    ...
    assert_output_bundle_state_v96((final_path, evidence_path), complete=False)
    ...
    # Validate both exact serializations and their cross-file binding before
    # either create-once official path is published.
    final_path.parent.mkdir(parents=True, exist_ok=True)
    evidence_path.parent.mkdir(parents=True, exist_ok=True)
    physical_path_v96_v2(final_path)
    physical_path_v96_v2(evidence_path)
    with tempfile.TemporaryDirectory(
        prefix=".v96-v2-final-prevalidate-", dir=final_path.parent
    ) as temporary_directory:
        staging = Path(temporary_directory)
        staged_final = staging / final_path.name
        staged_evidence = staging / evidence_path.name
        write_json_create_once_v96(staged_final, final)
        evidence_with_final = {
            **evidence,
            "final_score_sha256": sha256_file_v85(staged_final),
        }
        assert_aggregate_only_v96(evidence_with_final)
        write_json_create_once_v96(staged_evidence, evidence_with_final)
        if (
            read_json_strict_v96(staged_final) != final
            or read_json_strict_v96(staged_evidence) != evidence_with_final
        ):
            raise RuntimeError("V96 v2 final/evidence staging validation failed")
    write_json_create_once_v96(final_path, final)
    evidence = {**evidence, "final_score_sha256": sha256_file_v85(final_path)}
    if evidence != evidence_with_final:
        raise RuntimeError("V96 v2 final serialization changed after staging")
    assert_aggregate_only_v96(evidence)
    write_json_create_once_v96(evidence_path, evidence)
    return {**final, "evidence_sha256": sha256_file_v85(evidence_path)}
...
def authenticate_final_evidence_v96(
    ...
    final_path = physical_path_v96_v2(paths.final_score)
    evidence_path = physical_path_v96_v2(paths.evidence)
    assert_output_bundle_state_v96((final_path, evidence_path), complete=True)
    structured = authenticate_structured_score_v96(
        config_path, prediction_bundle=bundle
    )
    nll = authenticate_nll_bundle_v96(
    ...
    final = read_json_strict_v96(final_path)
    evidence = read_json_strict_v96(evidence_path)
    expected_evidence = {
        **expected_evidence,
        "final_score_sha256": sha256_file_v85(final_path),
    }
    if final != expected_final or evidence != expected_evidence:
        raise ValueError("V96 v2 final known-development evidence changed")
    assert_aggregate_only_v96(final)
    assert_aggregate_only_v96(evidence)
    return {
        **final,
        "final_score_sha256": sha256_file_v85(final_path),
        "evidence_sha256": sha256_file_v85(evidence_path),
        "authenticated": True,
    }
