"""Fail-closed authorization and final selection through V20 update 8.
...
from semantic_3d_chat.evaluation.v20_epoch_selector import (
    EXPECTED_EPOCHS,
    EXPECTED_FROZEN_GLOBAL_RESIDUAL_SHA256,
    OUTPUT_NAMESPACE,
    PINNED_CONFIG_PATH,
    V20EpochSelectorViolation,
    _color_eligible,
...
EXTENSION_NAMESPACE = "gemma4_color_mirror_signed_x_local_field_v20_extension_u8"
CONTROLLER_TYPE = "strict_v20_conditional_extension_controller"
FINAL_SELECTOR_TYPE = "strict_v20_conditional_extension_final_selector"
...
class V20ExtensionViolation(ValueError):
    """A mismatch that denies a V20 conditional launch or final selection."""


def _fail(message: str) -> None:
    raise V20ExtensionViolation(message)
...
def _file_sha256(path: Path, field: str) -> str:
    ...
    except V20EpochSelectorViolation as error:
        _fail(str(error))
...
def _load_exact_screen_report(
    ...
    try:
        ...
        recomputed = summarize_v20_epochs(
            config,
            selection,
        ...
        _fail(f"Cannot validate exact V20 screen report: {error}")
    if screen != recomputed:
        _fail("V20 screen differs from exact recomputation of its bound artifacts")
...
def _require_extension_decision(screen: Mapping[str, Any]) -> int:
    for key, expected in {
        ...
        if screen.get(key) != expected:
            _fail(f"screen.{key} does not authorize V20 continuation")
    _exact_int(
...
def _require_current_source(
    ...
    try:
        ...
        _fail(f"V20 extension requires clean committed source: {error}")
    if current != screen.get("source_provenance"):
        _fail("Current clean source provenance differs from the exact V20 screen")
    return current
...
def _build_launch_manifest(
    *,
    ...
    if extension_root == original_root or extension_root.is_relative_to(original_root):
        _fail("V20 extension namespace is not isolated")
    if require_namespace_absent and extension_root.exists():
        _fail(f"Refusing to reuse or overwrite existing V20 extension namespace: {extension_root}")
    ...
    if selected_hashes != screen.get("selected_checkpoint_artifact_hashes"):
        _fail("Selected checkpoint adapter/metadata/optimizer no longer match the V20 screen")
    if selected_hashes["metadata_sha256"] != screen.get("selected_checkpoint_metadata_sha256"):
        _fail("Selected checkpoint metadata alias no longer matches the V20 screen")
...
def prepare_extension_launch(
    ...
    *,
    ...
    """Authorize, but never execute, the isolated V20 update-8 continuation."""
...
def _validate_launch_manifest(
    ...
    *,
    ...
    except (V20EpochSelectorViolation, FileNotFoundError, OSError, UnicodeError) as error:
        _fail(f"Cannot load V20 extension launch manifest: {error}")
    manifest = dict(raw_manifest)
    if manifest.get("controller_type") != CONTROLLER_TYPE or manifest.get("authorized") is not True:
        _fail("V20 launch manifest is not an authorization from this controller")
    config_path = manifest.get("config_path")
    screen_path = manifest.get("screen_report_path")
    if not isinstance(config_path, str) or not isinstance(screen_path, str):
        _fail("V20 launch config/screen path is invalid")
    screen, screen_sha256, config, _selection = _load_exact_screen_report(config_path, screen_path)
    source = _require_current_source(screen, current_provenance)
    ...
    if manifest != expected:
        _fail("V20 launch manifest differs from exact current authorization")
...
def _validate_extension_epoch(
    ...
    try:
        ...
        _fail(f"Cannot load V20 extension update {epoch}: {error}")
    artifact = dict(raw)
    ...
    try:
        ...
        _fail(f"Extension update {epoch} violates the strict V20 contract: {error}")
    try:
        ...
        _fail(f"Extension update {epoch} artifact state violates V20: {error}")
...
def select_final_extension(
    ...
    *,
    ...
    if extension_root == original_root.resolve() or extension_root.name != EXTENSION_NAMESPACE:
        _fail("Launch extension root is not the exact isolated V20 namespace")
    expected_epochs = list(range(selected_epoch + 1, TARGET_OPTIMIZER_UPDATE + 1))
    ...
    try:
        ...
        _fail(f"Cannot reload selected V20 checkpoint metadata: {error}")
    ...
    if selected_hashes != manifest.get("selected_checkpoint_artifact_hashes"):
        _fail("Selected V20 checkpoint changed after extension authorization")
    ...
    if [candidate["epoch"] for candidate in candidates] != list(
        ...
        _fail("Final V20 trajectory does not cover optimizer updates 1 through 8")
    ...
    for candidate, state_hash in zip(candidates, state_hashes, strict=True):
        ...
        if prior is None:
            first_seen[state_hash] = epoch
            continue
        if prior != epoch - 1 or candidate["full_teacher_gate_passed"] is not True:
            _fail("V20 trajectory repeats or rolls back state before a full-teacher plateau")
    ...
    if not ranking:
        _fail("Final V20 trajectory has no color-eligible checkpoint")
