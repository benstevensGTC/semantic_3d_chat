"""Immutable staged authorization chain for Gemma tool-decoder V2.1."""
...
MPS_SMOKE_RELEASE_PATH: Final[str] = (
    "reports/gemma4/metrics/"
    "gemma4_embodied_tool_decoder_mps_smoke_release_v2_1.json"
)
MPS_SMOKE_REPORT_PATH: Final[str] = (
    "reports/gemma4/metrics/"
    "gemma4_embodied_tool_decoder_full_mps_smoke_v2_1.json"
)
TRAINING_RELEASE_PATH: Final[str] = (
    "reports/gemma4/metrics/"
    "gemma4_embodied_tool_decoder_multi_update_release_v2_1.json"
)
OLD_CPU_AUTHORIZATION_PATH: Final[str] = (
    "reports/gemma4/metrics/"
    "gemma4_embodied_tool_decoder_training_authorization_v2.json"
)
OLD_CPU_AUTHORIZATION_SHA256: Final[str] = (
    "73e7c8473a9006c4d13382a79a8436c2812e3c6ae99039d70fc42f2f9a422960"
)
EVALUATION_V2_1_SHA256: Final[str] = (
    "7b322d57ed46d920f7253383e75254b4157ac5397afc172bf7ffd0141310e007"
)
ARTIFACT: Final[str] = "gemma4_embodied_tool_decoder_training_authorization_v2_1"
BOUND_SOURCE_PATHS: Final[tuple[str, ...]] = (
    ...
    (
        "src/semantic_3d_chat/evaluation/"
        "gemma4_tool_decoder_training_authorization_v2_1.py"
    ),
    ...
    "scripts/authorize_gemma4_tool_decoder_v2_1_cpu.py",
    "scripts/release_gemma4_tool_decoder_v2_1_mps_smoke.py",
    "scripts/run_gemma4_tool_decoder_v2_1_mps_smoke.py",
    "scripts/release_gemma4_tool_decoder_v2_1_training.py",
...
def source_hashes_v2_1() -> dict[str, str]:
...
def _strict_json(path: Path) -> dict[str, Any]:
    def unique(pairs: list[tuple[str, Any]]) -> dict[str, Any]:
        ...
        for key, value in pairs:
            if key in output:
                raise ValueError(f"Duplicate V2.1 authorization field: {key}")
            output[key] = value
        return output

    value = json.loads(path.read_text(encoding="utf-8"), object_pairs_hook=unique)
    if not isinstance(value, dict) or set(value) != set(AUTHORIZATION_FIELDS):
        raise ValueError("V2.1 authorization field inventory changed")
    return value
...
def _strict_report_json(path: Path) -> dict[str, Any]:
    def unique(pairs: list[tuple[str, Any]]) -> dict[str, Any]:
        ...
        for key, value in pairs:
            if key in output:
                raise ValueError(f"Duplicate V2.1 smoke report field: {key}")
            output[key] = value
        return output

    def reject_constant(value: str) -> None:
        raise ValueError(f"Nonfinite V2.1 smoke report constant: {value}")
    ...
    if not isinstance(value, dict):
        raise TypeError("V2.1 MPS smoke report must be a mapping")
    return value


def load_authorization_payload_v2_1(path: str | Path) -> dict[str, Any]:
    """Read one exact-field authorization without following implicit state."""

    return _strict_json(_rooted(path))


def _base_payload() -> dict[str, Any]:
    old = PROJECT_ROOT / OLD_CPU_AUTHORIZATION_PATH
    if sha256_file(old) != OLD_CPU_AUTHORIZATION_SHA256:
        raise ValueError("Superseded V2 CPU authorization evidence changed")
    return {
        "schema_version": "2.1",
        "artifact": ARTIFACT,
        "supersedes_authorization_path": OLD_CPU_AUTHORIZATION_PATH,
        "supersedes_authorization_sha256": OLD_CPU_AUTHORIZATION_SHA256,
        "training_source_path": (
            "src/semantic_3d_chat/training/train_gemma4_tool_decoder_v2.py"
        ),
        ...
        "bound_source_sha256": source_hashes_v2_1(),
        "v1_terminal_failure_sha256": (
...
def build_cpu_authorization_v2_1() -> dict[str, Any]:
    return {
        **_base_payload(),
        "status": "cpu_inputs_sealed_heavy_mps_not_released",
        "authorization_stage": "cpu_preparation",
        "parent_authorization_path": OLD_CPU_AUTHORIZATION_PATH,
        "parent_authorization_sha256": OLD_CPU_AUTHORIZATION_SHA256,
...
def _authenticate_cpu_parent(path: str | Path) -> tuple[dict[str, Any], str]:
    ...
    if (
        payload != build_cpu_authorization_v2_1()
        ...
        raise ValueError("V2.1 CPU parent authorization changed")
    return payload, digest


def build_mps_smoke_release_v2_1(
...
def validate_mps_smoke_report_v2_1(
    ...
    *,
    ...
    if (
        report.get("schema")
        != "semantic_3d_chat.gemma4_tool_decoder_full_mps_smoke.v2_1"
        ...
        raise ValueError("V2.1 full-model MPS smoke report did not pass exact gates")
    return {
        "status": "passed",
        "device": "mps",
...
def build_training_release_v2_1(
    *,
    ...
    if release != build_mps_smoke_release_v2_1(
        release["parent_authorization_path"]
    ):
        raise ValueError("V2.1 MPS smoke release ancestry or bytes changed")
    report_path = _rooted(smoke_report)
    report_sha = sha256_file(report_path)
    report = _strict_report_json(report_path)
    summary = validate_mps_smoke_report_v2_1(
...
def _atomic_create(path: str | Path, payload: Mapping[str, Any]) -> tuple[Path, str]:
    ...
    if destination.exists():
        raise FileExistsError(f"V2.1 authorization is create-once: {destination}")
...
def write_cpu_authorization_v2_1(
    output: str | Path = CPU_AUTHORIZATION_PATH,
) -> tuple[Path, str]:
    return _atomic_create(output, build_cpu_authorization_v2_1())


def write_mps_smoke_release_v2_1(
    output: str | Path = MPS_SMOKE_RELEASE_PATH,
    *,
    cpu_authorization: str | Path = CPU_AUTHORIZATION_PATH,
) -> tuple[Path, str]:
    return _atomic_create(output, build_mps_smoke_release_v2_1(cpu_authorization))


def write_training_release_v2_1(
...
__all__ = [
    ...
    "build_cpu_authorization_v2_1",
    "build_mps_smoke_release_v2_1",
    "build_training_release_v2_1",
    "load_authorization_payload_v2_1",
    "sha256_file",
    "source_hashes_v2_1",
    "validate_mps_smoke_report_v2_1",
