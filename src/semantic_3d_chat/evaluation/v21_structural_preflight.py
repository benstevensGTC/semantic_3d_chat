"""Strict no-live-step phase-aware BF16 preflight for the V21 local field.

This supervised, offline diagnostic reconstructs the exact first V21 epoch
...
V21_PREFLIGHT_ROLE = "v21_exact_ordered_signed_x_local_field_phase_aware_structural_preflight"
V21_SCREEN_ROLE = "signed_x_local_field_phase_aware_screen"
V21_EXPERIMENT_ROLE = "exploratory_phase_aware_reflection_odd_local_field_screen_v21"
SIGNED_X_OPTIMIZER_GROUP_NAME = V19_SIGNED_X_OPTIMIZER_GROUP_NAME
COLOR_PAIR_ID = "pair_000001"
MIRROR_PAIR_ID = "pair_000003"

# The V21 curriculum intentionally inherits the already-audited V18 selection
...
EXPECTED_RESOLVED_CONFIG_HASH = "ae17da8b9a71"


class V21StructuralPreflightViolation(ValueError):
    """A fail-closed V21 configuration or evidence violation."""


def _fail(message: str) -> None:
    raise V21StructuralPreflightViolation(message)
...
def _validate_optimizer_contract(raw: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if normalized != pinned:
        _fail(f"V21 AdamW contract mismatch: expected={pinned} observed={normalized}")
    return normalized
...
def validate_v21_config_contract(
    ...
    *,
    ...
    if scene_encoder.get("global_latents") != 256:
        _fail("V21 requires exactly 256 globally complete scene slots")
    ...
    if (
        ...
        _fail("V21 requires the enabled 128D centered-content V18 residual base")
    if (
        ...
        _fail("V21 signed-X local-field architecture or initial-state pin mismatch")
    ...
    if failed_training:
        _fail(f"V21 training contract mismatch: {failed_training}")
    ...
    if (
        ...
        _fail("V21 pair curriculum no longer defines the exact twelve-step candidate schedule")
    ...
    if (
        ...
        _fail("V21 opaque per-pair objective policy mismatch")
    ...
    experiment_checks = {
        ...
        == EXPECTED_SOURCE_SCENE_STATE_SHA256,
        "source_runtime_scene_state_sha256": experiment.get("source_runtime_scene_state_sha256")
        == EXPECTED_SOURCE_SCENE_STATE_SHA256,
    ...
    expected_screen = {
        ...
        "predicted_update_requires": {
            ...
            "color_strict_positive_before_and_after": True,
            "mirror_weighted_margin_hinge_objective_strictly_improves": True,
    ...
    if failed_language:
        _fail(f"V21 language contract mismatch: {failed_language}")
    ...
    normalized = {
        ...
        "predicted_update_requires": expected_screen["predicted_update_requires"],
        "optimizer": optimizer,
        ...
        "expected_hashes": {
            ...
            "runtime_scene_state_sha256": EXPECTED_SOURCE_SCENE_STATE_SHA256,
...
def ordered_curriculum_evidence(curriculum: Sequence[Any]) -> tuple[list[dict[str, Any]], str]:
    """Return the exact opaque identity and hash of the V21 epoch-one order."""

    entries: list[dict[str, Any]] = []
    for microstep, batch in enumerate(curriculum, start=1):
        if getattr(batch, "kind", None) != "pair":
            _fail(f"V21 microstep {microstep} is not a pair batch")
        units = tuple(getattr(batch, "pair_units", ()))
        if len(units) != 1:
            _fail(f"V21 microstep {microstep} must contain exactly one pair unit")
        unit = units[0]
        entries.append(
            {
                "microstep": microstep,
...
def exact_clone_adamw_evidence(
    ...
    if [name for name, _parameter in live_named] != ["output_projection.weight"]:
        raise ValueError("V21 signed-X trainable surface is not exactly the output weight")
    if live_named[0][1].grad is None:
        raise ValueError("V21 signed-X clone simulation requires an accumulated gradient")
...
def functional_local_field_delta(
    ...
    *,
    ...
    """Return the FP32 delta and exact decoder-visible 16-bit cast delta.

    The scene tokenizer and both residual branches produce FP32 tokens.  The
    trainer and chat runtime apply V21 in FP32 and only then cast the completed
    scene tokens at the Gemma boundary.  The exact arithmetic is therefore
    ``Q16(base_fp32 + delta_fp32) - Q16(base_fp32)``. Casting either operand
    before the addition is not equivalent. V21 pins ``Q16`` to the stable BF16
    Gemma path. Accepting FP16 keeps this pure arithmetic helper useful for the
    recorded loader experiment and focused unit controls.
    """
    ...
    if model_dtype not in {torch.float16, torch.bfloat16}:
        raise ValueError("V21 structural preflight requires a float16 or bfloat16 boundary")
    if base_tokens.dtype is not torch.float32:
        raise ValueError("V21 structural preflight requires FP32 pre-Gemma scene tokens")
...
def precision_cast_audit(
    ...
    *,
    ...
    """Quantify exact 16-bit boundary survival and round-trip error."""
    ...
    return {
        "schema_version": 1,
        "algorithm": f"{dtype_label}_cast_of_fp32_base_plus_fp32_delta",
        ...
        "effective_delta_sha256": tensor_state_sha256(
            {"model_effective_delta_float32": effective_delta}
        ),
    }
...
def local_dependence_evidence(module: nn.Module) -> dict[str, Any]:
    ...
    path.  V19's global moment changes all output slots for these probes; V21's
...
def functional_unit_measurement(
    unit: Any,
    candidate_margins: torch.Tensor,
    full_vocab_margins: torch.Tensor,
) -> dict[str, Any]:
    """Bind one two-sided pair unit to its exact teacher-forced margins."""

    candidate = candidate_margins.detach().float().cpu().reshape(-1)
    full_vocab = full_vocab_margins.detach().float().cpu().reshape(-1)
    records = tuple(getattr(unit, "records", ()))
    if len(records) != 2 or candidate.numel() != 2 or full_vocab.numel() != 2:
        _fail("V21 functional evidence requires exactly two records and margins per unit")
    return {
        "pair_id": str(unit.pair_id),
        "question_key": str(unit.question_key),
        "sides": [
            {
                ...
                "candidate_margin": float(candidate[index]),
                "full_vocab_margin": float(full_vocab[index]),
            }
...
def evaluate_v21_structural_gate(
    ...
    *,
    ...
    phase_pair_diagnostics: Mapping[str, Mapping[str, Any]],
    precision_audits: Mapping[str, Mapping[str, Any]],
    structural_state: Mapping[str, Any],
    local_dependence: Mapping[str, Any],
    local_hidden_ranks: Mapping[str, Mapping[str, Any]],
    requirements: Mapping[str, Any],
) -> dict[str, Any]:
    """Apply V21's predeclared local-field, precision, and selectivity gate."""

    if not raw_scene_metrics or set(raw_scene_metrics) != set(effective_scene_metrics):
        raise ValueError("Raw/effective V21 scene metric sets must be equal and nonempty")
    if set(raw_scene_metrics) != set(precision_audits) or set(raw_scene_metrics) != set(
        local_hidden_ranks
    ):
        raise ValueError("V21 scene, precision-audit, and local-rank sets must be equal")
    if set(phase_pair_diagnostics) != {COLOR_PAIR_ID, MIRROR_PAIR_ID}:
        raise ValueError("V21 phase diagnostics must contain exactly color and mirror pairs")
    maximum_ratio = float(
    ...
    legacy_total_norm_diagnostic_only = requirements.get(
        "legacy_effective_total_norm_selectivity_diagnostic_only"
    )
    if legacy_total_norm_diagnostic_only is not True:
        raise ValueError(
            "V21 requires the legacy effective total-norm selectivity to be diagnostic-only"
        )
    minimum_raw_mirror_ratio = float(requirements["minimum_raw_mirror_residual_to_core_rms_ratio"])
    minimum_raw_selectivity = float(
        requirements["minimum_raw_mirror_to_color_normalized_selectivity"]
    )
    minimum_pair_cosine = float(requirements["minimum_mirror_effective_raw_pair_cosine"])
    minimum_aligned_gain = float(requirements["minimum_mirror_effective_raw_aligned_gain"])
    maximum_aligned_gain = float(requirements["maximum_mirror_effective_raw_aligned_gain"])
    minimum_signal_to_noise = float(requirements["minimum_mirror_signal_to_orthogonal_noise_ratio"])
    ...
    for scene_id in sorted(raw_scene_metrics):
        ...
        cast = precision_audits[scene_id]
        ...
        per_scene[scene_id] = {
            ...
            <= maximum_ratio,
            "model_precision_changed_nonzero": int(cast["changed_element_count"]) > 0,
            "model_precision_quantization_error_finite": math.isfinite(
                float(cast["quantization_error_rms"])
            ),
    ...
    observed_raw_selectivity = raw_selectivity["mirror_to_color_normalized_selectivity"]
    mirror_phase = _mapping(phase_pair_diagnostics[MIRROR_PAIR_ID], "phase_pair_diagnostics.mirror")
    mirror_actual = _mapping(mirror_phase.get("actual_pair"), "mirror_phase.actual_pair")
    cosine = mirror_actual.get("raw_effective_cosine")
    aligned_gain = mirror_actual.get("aligned_gain")
    aligned_rms = mirror_actual.get("aligned_effective_rms")
    orthogonal_rms = mirror_actual.get("orthogonal_quantization_rms")
    phase_scalars = (cosine, aligned_gain, aligned_rms, orthogonal_rms)
    phase_scalars_finite = all(
        isinstance(value, (int, float))
        and not isinstance(value, bool)
        and math.isfinite(float(value))
        for value in phase_scalars
    )
    signal_to_orthogonal = None
    orthogonal_noise_exact_zero = False
    if phase_scalars_finite:
        orthogonal_noise_exact_zero = float(orthogonal_rms) == 0.0
        if not orthogonal_noise_exact_zero:
            signal_to_orthogonal = abs(float(aligned_rms)) / float(orthogonal_rms)
    common_null = _mapping(mirror_phase.get("common_delta_null"), "mirror_phase.common_delta_null")
    shared_base = _mapping(mirror_phase.get("shared_base"), "mirror_phase.shared_base")
    phase_control_scalars_finite = all(
        isinstance(value, (int, float))
        and not isinstance(value, bool)
        and math.isfinite(float(value))
        and float(value) >= 0.0
        for value in (
            common_null.get("response_rms"),
            shared_base.get("phase_spread_rms"),
        )
    )
    selectivity_checks = {
        "raw_mirror_residual_at_least_minimum": float(
            raw_selectivity["mirror_residual_to_core_rms_ratio"]
        )
        >= minimum_raw_mirror_ratio,
        "raw_normalized_selectivity_at_least_minimum": (
            raw_selectivity["color_ratio_exact_zero"] is True
            and float(raw_selectivity["mirror_residual_to_core_rms_ratio"]) > 0.0
        )
        or (
            observed_raw_selectivity is not None
            and float(observed_raw_selectivity) >= minimum_raw_selectivity
        ),
    ...
    legacy_effective_total_norm_selectivity = {
        "diagnostic_only": True,
        "historical_reference_threshold": 1.5,
        "observed_mirror_to_color_normalized_selectivity": observed_selectivity,
        "would_have_passed_historical_v20_threshold": (
            observed_selectivity is not None and float(observed_selectivity) >= 1.5
        ),
        "excluded_from_v21_authorization": True,
    }
    phase_checks = {
        "algorithm_and_bfloat16_dtype_bound": (
            mirror_phase.get("algorithm_family") == PHASE_AWARE_PRECISION_PAIR_V1
            and mirror_phase.get("algorithm") == "phase_aware_bfloat16_pair_v1"
            and mirror_phase.get("model_dtype") == "bfloat16"
        ),
        "finite_signal_decomposition": phase_scalars_finite,
        "mirror_effective_raw_cosine_at_least_minimum": (
            phase_scalars_finite and float(cosine) >= minimum_pair_cosine
        ),
        "mirror_aligned_gain_in_range": (
            phase_scalars_finite
            and minimum_aligned_gain <= float(aligned_gain) <= maximum_aligned_gain
        ),
        "mirror_signal_to_orthogonal_noise_at_least_minimum": (
            phase_scalars_finite
            and (
                (orthogonal_noise_exact_zero and abs(float(aligned_rms)) > 0.0)
                or (
                    signal_to_orthogonal is not None
                    and signal_to_orthogonal >= minimum_signal_to_noise
                )
            )
        ),
        "shared_base_and_common_delta_controls_finite": phase_control_scalars_finite,
    }
    passed = (
        all_slots
        and local_structure
    ...
    return {
        ...
        "phase_alignment_checks": phase_checks,
        "mirror_signal_to_orthogonal_noise_ratio": signal_to_orthogonal,
        "mirror_orthogonal_noise_exact_zero": orthogonal_noise_exact_zero,
        "raw_pair_selectivity": raw_selectivity,
        "model_effective_pair_selectivity": effective_selectivity,
        "legacy_effective_total_norm_selectivity": legacy_effective_total_norm_selectivity,
        ...
        "maximum_observed_model_effective_delta_to_core_rms_ratio": max(
...
def run_preflight(
    ...
    *,
    ...
    try:
        ...
        _fail(f"V21 preflight requires clean committed source: {error}")
    ...
    if selection["selected_ids_sha256"] != EXPECTED_SELECTION_SHA256:
        _fail("V21 selected training-record hash mismatch")
    ...
    if pair_unit_selection_hash != EXPECTED_PAIR_UNIT_SELECTION_SHA256:
        _fail(
            "V21 selected pair-unit hash mismatch: "
    ...
    if tuple(sorted(by_scene)) != EXPECTED_SCENE_IDS:
        _fail(f"V21 selected scene mismatch: {sorted(by_scene)}")
    ...
    if len(curriculum) != 12 or ordered_hash != EXPECTED_ORDERED_UNIT_SHA256:
        _fail(
            "V21 ordered epoch-one unit mismatch: "
    ...
    if pair_membership_hash != EXPECTED_PAIR_MEMBERSHIP_SHA256:
        _fail("V21 pair-membership hash mismatch")
    ...
    if lora is None or lora.trainable_parameter_count != 0:
        _fail("V21 requires both named LoRA banks installed and entirely frozen")
    lora.eval()
    ...
    if len(feature_dims) != 1:
        _fail(f"V21 semantic feature dimensions differ: {sorted(feature_dims)}")
    ...
    if global_residual is None:
        _fail("V21 global residual construction returned None")
    ...
    if signed_residual is None:
        _fail("V21 signed-X residual construction returned None")
    ...
    if initial_global_hash != global_scene_residual_settings(config).expected_initial_state_sha256:
        _fail("V21 deterministic global residual initial-state mismatch before source load")
    ...
    if initial_signed_hash != EXPECTED_SIGNED_X_INITIAL_STATE_SHA256:
        _fail("V21 deterministic signed-X initial-state mismatch")
    if torch.count_nonzero(signed_residual.output_projection.weight).item() != 0:
        _fail("V21 signed-X output projection is not exact zero at construction")
    ...
    if source_artifact_hashes != {
        ...
        _fail(f"V21 source artifact hash mismatch: {source_artifact_hashes}")
    scene_state_modules = {
    ...
    if source_metadata.get("epoch") != 4:
        _fail("V21 must initialize from exact V18 epoch four")
    if source_metadata.get("frozen_scene_state_sha256") != EXPECTED_SOURCE_SCENE_STATE_SHA256:
        _fail("V21 source checkpoint scene-state metadata hash mismatch")
    if (
        source_metadata.get("global_scene_residual")
        != global_scene_residual_settings(config).contract()
    ):
        _fail("V21 source global residual metadata contract mismatch")
    ...
    if (
        composer.scene_start.dtype is not torch.bfloat16
        or composer.scene_end.dtype is not torch.bfloat16
    ):
        _fail("V21 native Gemma boundary embeddings must remain in checkpoint BF16")
    validate_global_scene_residual_state(
        global_residual,
        expected_parameter_count=400_128,
        context="V21 preflight frozen V18 source",
    )
    validate_signed_x_scene_residual_state(
        signed_residual,
        expected_parameter_count=196_608,
        context="V21 preflight fresh signed-X branch",
    )
    ...
    if observed_source_hashes != expected_source_hashes:
        _fail(
            "V21 loaded source state hash mismatch: "
    ...
    if [
        ...
        _fail("V21 fresh branch trainable surface is not exactly its output projection")
    structural_state = signed_residual.validate_structural_state()
    if structural_state.get("parameter_count") != 196_608:
        _fail("V21 signed-X structural parameter count mismatch")
    ...
    if not zero_equivalence["verified"]:
        _fail("V21 fresh signed-X branch failed exact step-zero identity")
    ...
    for microstep, batch in enumerate(curriculum, start=1):
        ...
        if not isinstance(margins, torch.Tensor):
            _fail("V21 pair objective did not return differentiable candidate margins")
        ...
        if not isinstance(full_vocab_loss, torch.Tensor) or not isinstance(
            ...
            _fail("V21 pair objective did not return full-vocabulary ranking tensors")
        ...
        if output_parameter.grad is None:
            _fail(f"V21 microstep {microstep} produced no signed-X gradient tensor")
        ...
        if unit.pair_id == COLOR_PAIR_ID:
            ...
            _fail(f"Unexpected V21 opaque pair ID: {unit.pair_id}")
    ...
    if final_gradient is None:
        _fail("V21 accumulated gradient disappeared")
    ...
    predicted_residual = copy.deepcopy(signed_residual)
    predicted_residual.zero_grad(set_to_none=True)
    with torch.no_grad():
        predicted_residual.output_projection.weight.copy_(simulated_weight)
    predicted_residual.requires_grad_(False).eval()
    predicted_residual_hash = module_collection_state_sha256(
        {"signed_x_scene_residual": predicted_residual}
    )
    if predicted_residual_hash != clone_evidence["predicted_signed_x_scene_residual_state_sha256"]:
        _fail("V21 isolated predicted module does not match the exact AdamW clone hash")

    functional_measurements: dict[str, list[dict[str, Any]]] = {
        "before": [],
        "after": [],
    }
    with torch.no_grad():
        predicted_outputs = {
            scene_id: apply_signed_x_scene_residual(
                base_outputs[scene_id], predicted_residual, centered_content[scene_id]
            )
            for scene_id in EXPECTED_SCENE_IDS
        }
        for phase, phase_outputs in (
            ("before", base_outputs),
            ("after", predicted_outputs),
        ):
            for batch in curriculum:
                unit = batch.pair_units[0]
                policy = pair_policies.resolve(unit.pair_id)
                outputs = {scene_id: phase_outputs[scene_id] for scene_id in unit.scene_ids}
                ...
                margins = diagnostics["margins"]
                full_vocab_margins = diagnostics["first_answer_token_full_vocab_margins"]
                if not isinstance(margins, torch.Tensor) or not isinstance(
                    full_vocab_margins, torch.Tensor
                ):
                    _fail("V21 predicted functional pass did not return margin tensors")
                functional_measurements[phase].append(
                    functional_unit_measurement(unit, margins, full_vocab_margins)
                )
                del diagnostics, outputs, margins, full_vocab_margins

    functional_policies = {
        pair_id: {
            "candidate_target_margin": float(pair_policies.resolve(pair_id).candidate_margin),
            "candidate_hinge_weight": float(pair_policies.resolve(pair_id).candidate_hinge_weight),
            "full_vocab_target_margin": float(pair_policies.resolve(pair_id).full_vocab_margin),
            "full_vocab_hinge_weight": float(
                pair_policies.resolve(pair_id).full_vocab_hinge_weight
            ),
        }
        for pair_id in (COLOR_PAIR_ID, MIRROR_PAIR_ID)
    }
    predicted_functional_audit = evaluate_v21_predicted_update(
        functional_measurements["before"],
        functional_measurements["after"],
        policies=functional_policies,
        color_pair_id=COLOR_PAIR_ID,
        mirror_pair_id=MIRROR_PAIR_ID,
        expected_units_per_pair=int(
            contract["predicted_update_requires"]["expected_units_per_pair"]
        ),
    )
    raw_scene_metrics: dict[str, dict[str, Any]] = {}
    effective_scene_metrics: dict[str, dict[str, Any]] = {}
    precision_audits: dict[str, dict[str, Any]] = {}
    raw_deltas: dict[str, torch.Tensor] = {}
    effective_deltas: dict[str, torch.Tensor] = {}
    model_base_tokens: dict[str, torch.Tensor] = {}
    model_dtype = language.model.get_input_embeddings().weight.dtype
    trainer_model_dtype = next(language.model.parameters()).dtype
    if model_dtype is not torch.bfloat16 or trainer_model_dtype is not model_dtype:
        _fail(
            "V21 expected matching bfloat16 Gemma input/trainer boundary dtypes; "
            f"input={model_dtype} trainer={trainer_model_dtype}"
        )
    with torch.no_grad():
        for scene_id in EXPECTED_SCENE_IDS:
            ...
            model_base = base_tokens.float().to(dtype=model_dtype).float()
            raw_deltas[scene_id] = raw_delta
            effective_deltas[scene_id] = effective_delta
            model_base_tokens[scene_id] = model_base
            precision_audits[scene_id] = precision_cast_audit(
                base_tokens,
                raw_delta,
                effective_delta,
                model_dtype=model_dtype,
            ...
            effective_scene_metrics[scene_id] = {
                **fp64_delta_metrics(model_base, effective_delta),
    ...
    effective_pair_metrics = {
        pair_id: {
            ...
            "second_scene_id": scene_pair_by_id[pair_id][1],
            **fp64_pair_delta_metrics(
                model_base_tokens[scene_pair_by_id[pair_id][0]],
                model_base_tokens[scene_pair_by_id[pair_id][1]],
    ...
    phase_pair_evidence = {
        pair_id: phase_aware_pair_diagnostics(
            base_outputs[scene_pair_by_id[pair_id][0]].scene_tokens,
            base_outputs[scene_pair_by_id[pair_id][1]].scene_tokens,
            raw_deltas[scene_pair_by_id[pair_id][0]],
            raw_deltas[scene_pair_by_id[pair_id][1]],
    ...
    authorization_checks = {
        ...
        "predicted_teacher_forced_functional_gate": bool(predicted_functional_audit["passed"]),
        "local_field_rank_precision_phase_gate": bool(structural_gate["passed"]),
    ...
    phase_audit_implementation_path = Path(
        __import__(
            "semantic_3d_chat.evaluation.v21_phase_aware_precision",
            fromlist=["__file__"],
        ).__file__
    ).resolve()
    functional_audit_implementation_path = Path(
        __import__(
            "semantic_3d_chat.evaluation.v21_predicted_update_audit",
            fromlist=["__file__"],
        ).__file__
    ).resolve()
    frozen_state_hashes = {
    ...
    report = {
        ...
        "question_dependent_scene_processing": False,
        "model_dtype": model_dtype_label(model_dtype),
        ...
        "phase_audit_implementation_source": str(
            phase_audit_implementation_path.relative_to(PROJECT_ROOT)
        ),
        "phase_audit_implementation_source_sha256": file_sha256(phase_audit_implementation_path),
        "functional_audit_implementation_source": str(
            functional_audit_implementation_path.relative_to(PROJECT_ROOT)
        ),
        "functional_audit_implementation_source_sha256": file_sha256(
            functional_audit_implementation_path
        ),
        ...
        "source_metadata_scene_state_sha256": source_metadata.get("frozen_scene_state_sha256"),
        "source_metadata_lora_bank_state_sha256": source_metadata.get("lora_bank_state_sha256"),
        "source_precision_transition": {
            "checkpoint_scene_state_sha256": EXPECTED_SOURCE_SCENE_STATE_SHA256,
            "runtime_scene_state_sha256": observed_source_hashes["scene_state_sha256"],
            "native_boundary_checkpoint_dtype": "bfloat16",
            "native_boundary_runtime_dtype": "bfloat16",
            "conversion_performed": False,
            "state_exactly_preserved": (
                observed_source_hashes["scene_state_sha256"] == EXPECTED_SOURCE_SCENE_STATE_SHA256
            ),
        },
        ...
        "predicted_update_functional_audit": predicted_functional_audit,
        ...
        "precision_cast_audit": precision_audits,
        "model_effective_scene_delta": effective_scene_metrics,
        "effective_cast_scene_delta": effective_scene_metrics,
        "raw_fp32_centered_pair_delta": raw_pair_metrics,
        "model_effective_pair_delta": effective_pair_metrics,
        "effective_cast_pair_delta": effective_pair_metrics,
        "phase_aware_pair_diagnostics": phase_pair_evidence,
