"""Audited local CLI for the promoted V85 strict continuous-memory runtime."""
...
def _parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--config", default="configs/runtime/gemma4_v85_strict_multiscene.yaml"
    )
    ...
    parser.add_argument(
        ...
        default=(
            ...
            "gemma4_v85_strict_multiscene_release_v1"
        ),
    )
    parser.add_argument(
        "--scene-memory", default="data_gemma4/runtime/scene_memories/v85/scene_000001"
    )
...
def _forbidden_roots() -> list[Path]:
    return [
        ...
        PROJECT_ROOT / "reports/gemma4/predictions/gemma4_v85_strict_multiscene_development.json",
        PROJECT_ROOT / "reports/gemma4/metrics/gemma4_v85_strict_multiscene_preregistration.json",
        PROJECT_ROOT / "reports/gemma4/metrics/gemma4_v85_strict_multiscene_training.json",
        PROJECT_ROOT / "reports/gemma4/metrics/gemma4_v85_strict_multiscene_development.json",
        PROJECT_ROOT / "reports/gemma4/artifacts/v85_strict_multiscene_final",
        PROJECT_ROOT / "reports/gemma4/artifacts/v75_fixed_atlas_historical_internal_v1/probe_bank",
...
def _run(argv: list[str] | None = None) -> int:
    ...
    audit_path = _rooted(
        args.audit_log or f"reports/gemma4/metrics/v85_chat_access_{args.scene}.json"
    )
    chat_path = _rooted(
        args.chat_log or f"reports/gemma4/examples/v85_chat_{args.scene}.jsonl"
    )
    completed = False
    ...
    try:
        with audit:
            ...
            from semantic_3d_chat.chat.v85_strict_multiscene_runtime import (
            ...
            if args.audit_log is None:
                audit_path = reports_root(config) / "metrics" / f"v85_chat_access_{args.scene}.json"
            if args.chat_log is None:
                chat_path = reports_root(config) / "examples" / f"v85_chat_{args.scene}.jsonl"
            runtime = V85StrictMultisceneChatRuntime.load(
                config,
                args.scene,
            ...
            if runtime.questions_answered != 0:
                raise RuntimeError("V85 scene memory was not bound before question input")
            if not runtime.runtime_promotion_authorized and not args.allow_candidate:
                raise RuntimeError("V85 checkpoint is not the promoted strict release")
            ...
            if len(set(runtime.environment_conditioned_input_hashes)) > 1:
                raise RuntimeError("V85 total environmental input changed across questions")
            completed = True
    finally:
...
def main(argv: list[str] | None = None) -> int:
    try:
        ...
        print(f"V85 chat refused: {error}", file=sys.stderr)
        return 2
