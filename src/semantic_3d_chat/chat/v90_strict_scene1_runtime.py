"""Strict V90 chat over one immutable pre-question continuous scene memory.

This runtime-only module accepts an already promoted V90 checkpoint.  It
imports no V90 preflight, trainer, evaluator, prediction, release, question,
...
The final V90 bridge digest is intentionally not a source constant.  A future
release packager must bind one lowercase SHA-256 digest identically across the
standalone runtime config, sanitized checkpoint metadata, bank-state table,
and authenticated release provenance before this class will start.
"""
...
RUNTIME_KIND: Final[str] = "v90_strict_scene1_direct_scene_memory"
...
V90_RANK: Final[int] = 8
V90_ALPHA: Final[float] = 16.0
...
_BANK_SPECS: Final[tuple[_BankSpec, ...]] = (
    ...
    _BankSpec(V90_BANK, (V90_TARGET,), V90_RANK, V90_ALPHA, V90_PARAMETER_COUNT, None),
)
...
def _mapping(value: object, label: str) -> Mapping[str, Any]:
    if not isinstance(value, Mapping):
        raise TypeError(f"V90 runtime {label} is malformed")
    return value


def validate_v90_runtime_contract(
    *,
    scene_id: str,
    ...
    if scene_id != SCENE_ID:
        raise ValueError("V90 strict scene-one runtime accepts only scene_000001")
    ...
    if not isinstance(banks_raw, list) or not all(
        ...
        raise TypeError("V90 runtime checkpoint bank inventory is malformed")
    ...
    if (
        ...
        raise ValueError("V90 runtime requires the exact ordered frozen 12-bank stack")

    metadata_by_name = {str(row["name"]): row for row in banks}
    v90_state = states.get(V90_BANK)
    if not _lower_sha256(v90_state):
        raise ValueError("V90 bridge state must be a lowercase SHA-256 digest")
    for spec in _BANK_SPECS:
        ...
        expected_state = spec.fixed_state_sha256 or str(v90_state)
        if (
            ...
            raise ValueError(f"V90 runtime frozen bank changed: {spec.name}")
    ...
    release = _mapping(
        provenance_root.get("v90_strict_runtime_release"), "release provenance"
    )
    if any(not _lower_sha256(release.get(key)) for key in _REQUIRED_RELEASE_HASHES):
        raise ValueError("V90 runtime release hash bindings are incomplete")
    if (
        release.get("schema_version") != 90
        or release.get("v90_bridge_state_sha256") != v90_state
        ...
        raise ValueError("V90 runtime is not bound to the promoted conversational gate")
    return {
        "v90_bridge_state_sha256": str(v90_state),
...
class V90StrictScene1ChatRuntime(V83DirectSceneMemoryChatRuntime):
    """Direct 738-token memory with the exact frozen twelve-bank V90 stack."""
    ...
    def __init__(self, base: StaticChatRuntime, loaded: LoadedV81SceneMemory) -> None:
        ...
        if (
            ...
            raise ValueError("V90 requires the exact oracle-free scene-one 738-token memory")
        contract = validate_v90_runtime_contract(
        ...
        self.v90_bridge_state_sha256 = contract["v90_bridge_state_sha256"]
    ...
    def startup_summary(self) -> dict[str, Any]:
        ...
        return {
            **summary,
            "phase": "v90_strict_scene1_ready",
            "runtime_kind": RUNTIME_KIND,
            "frozen_lora_bank_count": 12,
            "frozen_lora_parameter_count": EXPECTED_ADAPTER_PARAMETER_COUNT,
            "trainable_runtime_parameter_count": 0,
            "v90_bridge_bank": V90_BANK,
            "v90_bridge_target": V90_TARGET,
            "v90_bridge_state_sha256": self.v90_bridge_state_sha256,
    ...
    def answer(self, question: str) -> ChatAnswer:
        ...
        if self.last_prepared_layout_audit is None:
            raise RuntimeError("V90 generation did not produce a direct-memory layout audit")
        if result.prefix_hash != self.scene_prefix_hash:
            raise RuntimeError("V90 total environment-conditioned input changed")
        ...
        if len(set(self.environment_conditioned_input_hashes)) != 1:
            raise RuntimeError("V90 environment-conditioned input is question-dependent")
        return result
