"""Audited local CLI for an authenticated promoted V92 scene-one release."""
...
DEFAULT_CONFIG: Final[str] = "configs/runtime/gemma4_v92_strict_scene1.yaml"
...
_OFFLINE_PATHS: Final[tuple[str, ...]] = (
    ...
    "data_gemma4/checkpoints/v92_scene1_retention_conversation_repair_work",
...
def _forbidden_roots() -> list[Path]:
    ...
    return [PROJECT_ROOT / path for path in (*inherited, *_OFFLINE_PATHS)]


def _run(argv: list[str] | None = None) -> int:
    args = _parser().parse_args(argv)
    if args.scene != "scene_000001":
        raise ValueError("V92 strict scene-one chat accepts only scene_000001")
    ...
    audit_path = _rooted(
        args.audit_log or f"reports/gemma4/metrics/v92_chat_access_{args.scene}.json"
    )
    chat_path = _rooted(
        args.chat_log or f"reports/gemma4/examples/v92_chat_{args.scene}.jsonl"
    )
    completed = False
        ...
        with audit:
            ...
            if args.audit_log is None:
                audit_path = (
                    reports_root(config)
                    / "metrics"
                    / f"v92_chat_access_{args.scene}.json"
                )
            if args.chat_log is None:
                chat_path = (
                    reports_root(config)
                    / "examples"
                    / f"v92_chat_{args.scene}.jsonl"
                )
            runtime = V92StrictScene1ChatRuntime.load(
                config,
                args.scene,
            ...
            if runtime.questions_answered != 0:
                raise RuntimeError("V92 scene memory was not bound before question input")
            if not runtime.runtime_promotion_authorized:
                raise RuntimeError("V92 checkpoint is not the promoted strict release")
            startup = {
            ...
            if len(set(runtime.environment_conditioned_input_hashes)) > 1:
                raise RuntimeError("V92 total environmental input changed across questions")
            completed = True
    finally:
...
def main(argv: list[str] | None = None) -> int:
    try:
        ...
        print(f"V92 chat refused: {error}", file=sys.stderr)
        return 2
