"""Audited local chat over a sealed V81 continuous 3D scene memory."""
...
def _parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        help="Optional authenticated V78 all-scene-token numeric grounding sidecar.",
    )
...
def _run(argv: list[str] | None = None) -> int:
    ...
    audit_path = _rooted(
        args.audit_log or "reports/gemma4/metrics/v81_chat_access.json"
    )
    chat_path = _rooted(args.chat_log or "reports/gemma4/examples/v81_chat.jsonl")
    completed = False
    try:
        with audit:
            ...
            if args.audit_log is None:
                audit_path = reports_root(config) / "metrics" / "v81_chat_access.json"
            if args.chat_log is None:
                chat_path = reports_root(config) / "examples" / "v81_chat.jsonl"
            ...
            if runtime.questions_answered != 0:
                raise RuntimeError("V81 fixed memory was not bound before question input")
...
def main(argv: list[str] | None = None) -> int:
    try:
        ...
        print(f"V81 chat refused: {error}", file=sys.stderr)
        return 2
