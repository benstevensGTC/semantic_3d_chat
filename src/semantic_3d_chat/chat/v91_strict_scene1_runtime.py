"""Strict V91 chat over one immutable pre-question continuous scene memory.

This runtime-only module accepts an already promoted V91 checkpoint.  It
imports no preflight, trainer, evaluator, prediction, release, question,
answer, or oracle surface.  Environmental information reaches local Gemma
only through the complete numeric ``[1, 738, 1536]`` scene memory inherited
from the direct-memory runtime.

The failed-but-authenticated V90 bridge is a fixed frozen parent bank.  The
final V91 bridge digest is deliberately dynamic: a release packager must bind
it identically across the standalone runtime config, sanitized checkpoint,
bank-state table, and authenticated release provenance before this class can
start.
"""
...
RUNTIME_KIND: Final[str] = "v91_strict_scene1_direct_scene_memory"
...
V91_RANK: Final[int] = 16
V91_ALPHA: Final[float] = 32.0
...
PROMOTION_DECISION: Final[str] = "strict_scene1_conversational_repair_primary"
...
_BANK_SPECS: Final[tuple[_BankSpec, ...]] = (
    ...
    _BankSpec(V91_BANK, (V91_TARGET,), V91_RANK, V91_ALPHA, V91_PARAMETER_COUNT, None),
)
...
def _mapping(value: object, label: str) -> Mapping[str, Any]:
    if not isinstance(value, Mapping):
        raise TypeError(f"V91 runtime {label} is malformed")
    return value


def validate_v91_runtime_contract(
    *,
    scene_id: str,
    ...
    if scene_id != SCENE_ID:
        raise ValueError("V91 strict scene-one runtime accepts only scene_000001")
    ...
    if not isinstance(banks_raw, list) or not all(
        ...
        raise TypeError("V91 runtime checkpoint bank inventory is malformed")
    ...
    if (
        ...
        raise ValueError("V91 runtime requires the exact ordered frozen 13-bank stack")

    metadata_by_name = {str(row["name"]): row for row in banks}
    v91_state = states.get(V91_BANK)
    if not _lower_sha256(v91_state):
        raise ValueError("V91 bridge state must be a lowercase SHA-256 digest")
    if states.get(V90_BANK) != V90_STATE_SHA256:
        raise ValueError(
            "V91 runtime requires the exact failed V90 bridge as frozen parent"
        )
    for spec in _BANK_SPECS:
        ...
        expected_state = spec.fixed_state_sha256 or str(v91_state)
        if (
            ...
            raise ValueError(f"V91 runtime frozen bank changed: {spec.name}")
    ...
    release = _mapping(
        provenance_root.get("v91_strict_runtime_release"), "release provenance"
    )
    if any(not _lower_sha256(release.get(key)) for key in _REQUIRED_RELEASE_HASHES):
        raise ValueError("V91 runtime release hash bindings are incomplete")
    if (
        release.get("schema_version") != 91
        or release.get("v90_bridge_state_sha256") != V90_STATE_SHA256
        or release.get("v91_bridge_state_sha256") != v91_state
        ...
        raise ValueError("V91 runtime is not bound to the promoted conversational gate")
    return {
        "v91_bridge_state_sha256": str(v91_state),
...
class V91StrictScene1ChatRuntime(V83DirectSceneMemoryChatRuntime):
    """Direct 738-token memory with the exact frozen thirteen-bank V91 stack."""
    ...
    def __init__(self, base: StaticChatRuntime, loaded: LoadedV81SceneMemory) -> None:
        ...
        if (
            ...
            raise ValueError("V91 requires the exact oracle-free scene-one 738-token memory")
        contract = validate_v91_runtime_contract(
        ...
        self.v91_bridge_state_sha256 = contract["v91_bridge_state_sha256"]
    ...
    def startup_summary(self) -> dict[str, Any]:
        ...
        return {
            **summary,
            "phase": "v91_strict_scene1_ready",
            "runtime_kind": RUNTIME_KIND,
            "frozen_lora_bank_count": 13,
            "frozen_lora_parameter_count": EXPECTED_ADAPTER_PARAMETER_COUNT,
            "trainable_runtime_parameter_count": 0,
            "v91_bridge_bank": V91_BANK,
            "v91_bridge_target": V91_TARGET,
            "v91_bridge_state_sha256": self.v91_bridge_state_sha256,
    ...
    def answer(self, question: str) -> ChatAnswer:
        ...
        if self.last_prepared_layout_audit is None:
            raise RuntimeError("V91 generation did not produce a direct-memory layout audit")
        if result.prefix_hash != self.scene_prefix_hash:
            raise RuntimeError("V91 total environment-conditioned input changed")
        ...
        if len(set(self.environment_conditioned_input_hashes)) != 1:
            raise RuntimeError("V91 environment-conditioned input is question-dependent")
        return result
