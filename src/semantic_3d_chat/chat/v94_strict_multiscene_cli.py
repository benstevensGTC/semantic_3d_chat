"""Audited local CLI for the strict V94 multi-scene continuous-memory runtime."""
...
DEFAULT_CONFIG: Final[str] = "configs/runtime/gemma4_v94_strict_multiscene.yaml"
DEFAULT_SCENE: Final[str] = "scene_000057"
RELEASE_CHECKPOINT: Final[str] = (
    "data_gemma4/runtime/checkpoints/"
    "gemma4_v94_strict_multiscene_release_v1"
)
CANDIDATE_CHECKPOINT: Final[str] = (
    "reports/gemma4/artifacts/v94_strict_runtime_candidate"
)
RELEASE_MEMORY_ROOT: Final[str] = "data_gemma4/runtime/scene_memories/v94"
CANDIDATE_MEMORY_ROOT: Final[str] = (
    "reports/gemma4/artifacts/v94_strict_runtime_candidate_memories"
)
...
def _forbidden_roots() -> list[Path]:
    """Return offline supervision, build, and scoring surfaces chat may not open."""
    ...
    roots = [
        ...
        PROJECT_ROOT / "data_gemma4/checkpoints/v94_strict_multiscene_full40_work",
        ...
        PROJECT_ROOT / "reports/gemma4/artifacts/v94_strict_multiscene_full40",
        PROJECT_ROOT / "reports/gemma4/predictions",
        PROJECT_ROOT
        / "reports/gemma4/metrics/gemma4_v94_strict_multiscene_full40_preregistration.json",
        PROJECT_ROOT
        / "reports/gemma4/metrics/gemma4_v94_strict_multiscene_full40_cpu_preflight.json",
        PROJECT_ROOT
        / "reports/gemma4/metrics/gemma4_v94_strict_multiscene_full40_training.json",
        PROJECT_ROOT
        / "reports/gemma4/metrics/gemma4_v94_strict_multiscene_full40_validation.json",
        PROJECT_ROOT / "reports/gemma4/metrics/gemma4_v94_strict_multiscene_evidence.json",
        ...
        PROJECT_ROOT / "reports/gemma4/artifacts/v75_fixed_atlas_historical_internal_v1",
    ]
...
def _run(argv: list[str] | None = None) -> int:
    ...
    audit_path = _rooted(
        args.audit_log or f"reports/gemma4/metrics/v94_chat_access_{args.scene}.json"
    )
    chat_path = _rooted(
        args.chat_log or f"reports/gemma4/examples/v94_chat_{args.scene}.jsonl"
    )
    completed = False
        ...
        with audit:
            ...
            if args.audit_log is None:
                audit_path = (
                    reports_root(config)
                    / "metrics"
                    / f"v94_chat_access_{args.scene}.json"
                )
            if args.chat_log is None:
                chat_path = (
                    reports_root(config)
                    / "examples"
                    / f"v94_chat_{args.scene}.jsonl"
                )
            runtime = V94StrictMultisceneChatRuntime.load(
                config,
                args.scene,
            ...
            if runtime.questions_answered != 0:
                raise RuntimeError("V94 scene memory was not bound before question input")
            ...
            if runtime_mode == "candidate" and not args.allow_candidate:
                raise RuntimeError(
                    "V94 checkpoint is a candidate; pass --allow-candidate explicitly"
                )
            if runtime_mode == "promoted" and not runtime_promoted:
                raise RuntimeError("V94 promoted provenance is internally inconsistent")
            startup = {
            ...
            if len(set(runtime.environment_conditioned_input_hashes)) > 1:
                raise RuntimeError("V94 environmental input changed across questions")
            completed = True
    finally:
...
def main(argv: list[str] | None = None) -> int:
    try:
        ...
        print(f"V94 chat refused: {error}", file=sys.stderr)
        return 2
