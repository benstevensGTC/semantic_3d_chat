"""Audited local CLI for the promoted V90 continuous-memory scene-one runtime."""
...
DEFAULT_CONFIG: Final[str] = "configs/runtime/gemma4_v90_strict_scene1.yaml"
...
DEFAULT_SCENE_MEMORY: Final[str] = (
    "data_gemma4/runtime/scene_memories/v90/scene_000001"
)
_V90_OFFLINE_PATHS: Final[tuple[str, ...]] = (
...
def _forbidden_roots() -> list[Path]:
    ...
    return [PROJECT_ROOT / path for path in (*inherited, *_V90_OFFLINE_PATHS)]


def _run(argv: list[str] | None = None) -> int:
    args = _parser().parse_args(argv)
    if args.scene != "scene_000001":
        raise ValueError("V90 strict scene-one chat accepts only scene_000001")
    ...
    audit_path = _rooted(
        args.audit_log or f"reports/gemma4/metrics/v90_chat_access_{args.scene}.json"
    )
    chat_path = _rooted(
        args.chat_log or f"reports/gemma4/examples/v90_chat_{args.scene}.jsonl"
    )
    completed = False
        ...
        with audit:
            ...
            if args.audit_log is None:
                audit_path = (
                    reports_root(config)
                    / "metrics"
                    / f"v90_chat_access_{args.scene}.json"
                )
            if args.chat_log is None:
                chat_path = (
                    reports_root(config)
                    / "examples"
                    / f"v90_chat_{args.scene}.jsonl"
                )
            runtime = V90StrictScene1ChatRuntime.load(
                config,
                args.scene,
            ...
            if runtime.questions_answered != 0:
                raise RuntimeError("V90 scene memory was not bound before question input")
            if not runtime.runtime_promotion_authorized:
                raise RuntimeError("V90 checkpoint is not the promoted strict release")
            startup = {
            ...
            if len(set(runtime.environment_conditioned_input_hashes)) > 1:
                raise RuntimeError("V90 total environmental input changed across questions")
            completed = True
    finally:
...
def main(argv: list[str] | None = None) -> int:
    try:
        ...
        print(f"V90 chat refused: {error}", file=sys.stderr)
        return 2
