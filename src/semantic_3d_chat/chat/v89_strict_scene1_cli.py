"""Audited local CLI for the V89 strict continuous-memory scene-one runtime."""
...
def _parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--config", default="configs/runtime/gemma4_v89_strict_scene1.yaml"
    )
    parser.add_argument("--scene", default="scene_000001")
    parser.add_argument(
        "--base-checkpoint",
        default="data_gemma4/runtime/checkpoints/gemma4_v89_strict_scene1_release_v1",
    )
    parser.add_argument(
        "--scene-memory",
        default="data_gemma4/runtime/scene_memories/v89/scene_000001",
    )
...
def _forbidden_roots() -> list[Path]:
    """Training, evaluation, and oracle surfaces the chat child may not open."""
    ...
    return [
        ...
        PROJECT_ROOT / "data_gemma4/checkpoints/v89_scene1_retention_work",
        ...
        PROJECT_ROOT
        / "reports/gemma4/metrics/gemma4_v89_scene1_retention_training.json",
        PROJECT_ROOT
...
def _run(argv: list[str] | None = None) -> int:
    ...
    if args.scene != "scene_000001":
        raise ValueError("V89 strict scene-one chat accepts only scene_000001")
    ...
    audit_path = _rooted(
        args.audit_log or f"reports/gemma4/metrics/v89_chat_access_{args.scene}.json"
    )
    chat_path = _rooted(
        args.chat_log or f"reports/gemma4/examples/v89_chat_{args.scene}.jsonl"
    )
    completed = False
        ...
        with audit:
            ...
            if args.audit_log is None:
                audit_path = (
                    reports_root(config)
                    / "metrics"
                    / f"v89_chat_access_{args.scene}.json"
                )
            if args.chat_log is None:
                chat_path = (
                    reports_root(config)
                    / "examples"
                    / f"v89_chat_{args.scene}.jsonl"
                )
            runtime = V89StrictScene1ChatRuntime.load(
                config,
                args.scene,
            ...
            if runtime.questions_answered != 0:
                raise RuntimeError(
                    "V89 scene memory was not bound before question input"
                )
            if not runtime.runtime_promotion_authorized and not args.allow_candidate:
                raise RuntimeError("V89 checkpoint is not the promoted strict release")
            ...
            if len(set(runtime.environment_conditioned_input_hashes)) > 1:
                raise RuntimeError(
                    "V89 total environmental input changed across questions"
                )
            completed = True
    finally:
...
def main(argv: list[str] | None = None) -> int:
    try:
        ...
        print(f"V89 chat refused: {error}", file=sys.stderr)
        return 2
