"""Audited local Gemma chat using the learned V82 sealed-memory reader."""
...
def _run(argv: list[str] | None = None) -> int:
    ...
    audit_path = _rooted(
        args.audit_log or "reports/gemma4/metrics/v82_chat_access.json"
    )
    chat_path = _rooted(args.chat_log or "reports/gemma4/examples/v82_chat.jsonl")
    completed = False
    try:
        with audit:
            ...
            if args.audit_log is None:
                audit_path = reports_root(config) / "metrics" / "v82_chat_access.json"
            if args.chat_log is None:
                chat_path = reports_root(config) / "examples" / "v82_chat.jsonl"
            runtime = V82SceneMemoryChatRuntime.load(
                config,
                args.scene,
            ...
            if runtime.questions_answered != 0:
                raise RuntimeError("V82 fixed memory was not bound before question input")
            ...
            def answer(question: str) -> None:
                ...
                payload = {
                    ...
                    **result.to_dict(),
                    ...
                    "reader_weights_sha256": runtime.learned_reader_metadata[
                        "weights_sha256"
                    ],
                    "reader_audit": runtime.last_reader_audit,
                    "grounding_audit": runtime.last_grounding_audit,
    ...
    if completed:
        print(
            json.dumps(
                {
                    "phase": "v82_chat_audit_complete",
                    "passed": True,
                    "fixed_memory_invariant": True,
                    "learned_reader_invariant": True,
                    "compiler_probe_or_training_cache_loaded": False,
...
def main(argv: list[str] | None = None) -> int:
    try:
        ...
        print(f"V82 chat refused: {error}", file=sys.stderr)
        return 2
