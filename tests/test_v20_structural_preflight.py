...
def test_current_v20_config_satisfies_strict_no_step_contract() -> None:
    contract = validate_v20_config_contract(load_config(CONFIG_PATH))

    assert contract["role"] == "v20_exact_ordered_signed_x_local_field_structural_preflight"
...
@pytest.mark.parametrize(
    ...
    [
        ...
        lambda config: config["v20_screen"]["structural_preflight_requires"].update(
...
def test_v20_contract_fails_closed_on_gradient_or_source_mutation(mutate) -> None:
    config = copy.deepcopy(load_config(CONFIG_PATH))
    mutate(config)

    with pytest.raises(V20StructuralPreflightViolation):
        validate_v20_config_contract(config)
...
def test_structural_gate_requires_rank_bfloat16_signal_and_selectivity() -> None:
    module = SignedXLocalFieldSceneResidual(scene_dim=16, latent_count=256, content_dim=8)
    ...
    effective_pairs = {
        "pair_000001": _pair_metric(0.01),
        "pair_000003": _pair_metric(0.02),
    }

    gate = evaluate_v20_structural_gate(
        raw,
        effective,
    ...
    assert gate["bf16_effective_pair_selectivity"]["mirror_to_color_normalized_selectivity"] == 2.0

    ranks["a"]["minimum_spatial_rank"] = 1
    failed = evaluate_v20_structural_gate(
        raw,
        effective,
        raw_pair_metrics=raw_pairs,
        effective_pair_metrics=effective_pairs,
