...
from semantic_3d_chat.evaluation.v87_accuracy_figure import (
    ANSWER_TYPE_ORDER,
...
def test_v87_figure_source_is_exact_terminal_evaluation() -> None:
    ...
    assert report["metrics"]["generic_smoke"]["correct"] == 0
    assert report["metrics"]["causal_control"]["canonical_prediction_changes"] == 2


def test_v87_figure_is_deterministic_and_scope_bounded(tmp_path: Path) -> None:
    ...
    assert first["metrics"]["overall"]["correct"] == 103
    assert first["metrics"]["acceptance_passed"] is False
...
def test_v87_figure_fails_before_parsing_modified_report(
    tmp_path: Path,
...
def test_default_v87_figure_and_summary_match_generated_hashes() -> None:
