...
from semantic_3d_chat.evaluation import v23_archive_validator as archive

PROJECT_ROOT = Path(__file__).resolve().parents[1]
SUMMARY_PATH = PROJECT_ROOT / archive.ARCHIVE_RELATIVE_PATH
FINAL_PATH = PROJECT_ROOT / "reports/gemma4/metrics/v23_extension_final.json"
CHECKPOINT_PATH = PROJECT_ROOT / "data_gemma4/checkpoints/gemma4_v23_shared_kv/epoch_002"
...
def test_tracked_v23_summary_has_exact_source_independent_seal() -> None:
...
def test_v23_summary_byte_tamper_fails_before_artifact_validation(tmp_path: Path) -> None:
    value = _summary()
    value["outcome"]["greedy_audit_authorized"] = True
    tampered = tmp_path / "v23_final_summary.json"
    tampered.write_text(json.dumps(value, indent=2) + "\n", encoding="utf-8")

    with pytest.raises(archive.V23ArchiveViolation, match="archive summary SHA-256"):
...
@pytest.mark.skipif(not EVIDENCE_AVAILABLE, reason="generated V23 evidence is not present")
def test_complete_v23_archive_validates_after_source_tree_changes() -> None:
    ...
    assert result["superseded_attempt_verified"] is True
...
@pytest.mark.skipif(not EVIDENCE_AVAILABLE, reason="generated V23 evidence is not present")
def test_bound_v23_final_hash_tamper_fails_closed(
    ...
    with pytest.raises(archive.V23ArchiveViolation, match="final_selector SHA-256"):
        archive.validate_archive(SUMMARY_PATH, repo_root=PROJECT_ROOT)


@pytest.mark.parametrize("forbidden_kind", ["promotion", "greedy", "static", "robot"])
def test_closed_v23_rejects_forbidden_downstream_outputs(
    forbidden_kind: str, tmp_path: Path
) -> None:
    checkpoint_root = tmp_path / "data_gemma4/checkpoints/v23"
    checkpoint_root.mkdir(parents=True)
    metrics = tmp_path / "reports/gemma4/metrics"
    metrics.mkdir(parents=True)
    forbidden = {
        "checkpoint_roots": ["data_gemma4/checkpoints/v23"],
    ...
    if forbidden_kind == "promotion":
        ...
        filename = {
            "greedy": "v23_greedy.json",
            "static": "v23_static_qa.json",
            "robot": "v23_robot.json",
        }[forbidden_kind]
        (metrics / filename).write_text("{}\n", encoding="utf-8")

    with pytest.raises(archive.V23ArchiveViolation, match="denied"):
        archive._validate_denial_absence(tmp_path, forbidden)


def test_v23_archive_path_cannot_escape_repository(tmp_path: Path) -> None:
    with pytest.raises(archive.V23ArchiveViolation, match="must remain inside"):
