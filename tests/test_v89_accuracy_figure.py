...
from semantic_3d_chat.evaluation.v89_accuracy_figure import (
    ANSWER_TYPE_ORDER,
...
def test_v89_figure_source_is_exact_passing_model_evaluation() -> None:
    ...
    assert report["metrics"]["model_acceptance_gate_passed"] is True
    assert all(report["metrics"]["model_acceptance_gates"].values())
...
def test_v89_figure_is_deterministic_and_scope_bounded(tmp_path: Path) -> None:
    ...
    assert first["scope"]["runtime_smoke_evidence"] is False
    assert first["scope"]["runtime_promotion_authorized"] is False
    assert first["metrics"]["acceptance_passed"] is True
    assert first["metrics"]["all_model_gates_passed"] is True
...
def test_v89_figure_fails_before_parsing_modified_report(
    tmp_path: Path,
...
def test_default_v89_figure_and_summary_match_generated_hashes() -> None:
